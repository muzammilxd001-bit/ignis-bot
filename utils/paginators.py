...
from .Paginator import Paginator as rayPaginator
...
class FieldPagePaginator(menus.ListPageSource):
    ...
    async def format_page(self, menu: rayPaginator,
        ...
        if maximum > 1:
            text = f'Ignis • Page {menu.current_page + 1}/{maximum}'
...
class TextPaginator(menus.ListPageSource):
    ...
    async def format_page(self, menu, content):
        maximum = self.get_max_pages()
        if maximum > 1:
            return f'{content}\nIgnis • Page {menu.current_page + 1}/{maximum}'
        return content
