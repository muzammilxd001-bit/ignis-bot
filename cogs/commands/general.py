...
password = [
    ...
    '471747183813474', '123747019', '312312318', '1x2xx3xx4xxxx'
]
...
Aliases""")
    ...
    async def _user(self,
        ...
        webp = avatar.replace(format='webp')
        jpg = avatar.replace(format='jpg')
        png = avatar.replace(format='png')
        embed = discord.Embed(
                color=0x2b2d31,
                description=f"[`PNG`]({png}) | [`JPG`]({jpg}) | [`WEBP`]({webp})"
                if not avatar.is_animated() else
                f"[`PNG`]({png}) | [`JPG`]({jpg}) | [`WEBP`]({webp}) | [`GIF`]({avatar.replace(format='gif')})"
            )
    ...
    async def membercount(self, ctx: commands.Context):
        ...
        embed = discord.Embed(
            ...
            description="<:Channel:1263146223364210801> **MemberCount **\n**<:member:1348326398929932388> %s members**" % (len(ctx.guild.members)),
    )
        if ctx.guild.icon:
    ...
    async def hack(self, ctx: commands.Context, member: discord.Member):
        ...
        embed = discord.Embed(
            title="**__HACKED !__**",
            description=f"**A dangerous hack on `{member.display_name}` has been successful. \n <:Latency:1263144230642122823> Details - \n IP : 192.168.1.128 \nCOUNTRY CODE : +91 \nPHONE NUMBER : ||Ignis ka Lund Lega||\n USER ID - {member.id} **",
          color=0x2b2d31
    )
        embed.set_footer(
    ...
    @commands.command(name="users", help="check users of Ignis .")
    @blacklist_check()
    @ignore_check()
    ...
    async def gender(self, ctx: commands.Context, member: discord.Member):
        embed = discord.Embed(description=f"{member.mention} is Gay",
    ...
    async def wizz(self, ctx: commands.Context):
        message6 = await ctx.send(
            f"`Wizzing {ctx.guild.name}, will take 21 seconds to complete`")
    ...
    async def pikachu(self, ctx: commands.Context):
        ...
        embed = discord.Embed(title='Pikachu',
                              ...
                              color=0x2b2d31)
        embed.set_image(url="https://some-random-api.ml/img/pikachu")
