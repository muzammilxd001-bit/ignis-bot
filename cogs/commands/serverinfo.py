...
class Info(commands.Cog):
    ...
    async def yomom(self, ctx: Context, *, role: discord.Role):
        ...
        embed.set_author(
            name=f"{role.name}'s Information", icon_url=ctx.bot.user.avatar
        )
        embed.add_field(
    ...
    @commands.command(
        name="serverinfo",
        aliases=["sinfo", "si"],
        description="Show's information of the server",
    )
    @blacklist_check()
    @ignore_check()
    async def serverinfo(self, ctx: commands.Context):
        ...
        if guild.system_channel_flags.join_notifications:
            haa = "**System Welcome Messages**: <:tick:1348326381611647046>"
        else:
            haa = "**System Welcome Messages**: <:cross:1348326385386651748>"
        afkk = ctx.guild.afk_channel
        if ctx.guild.system_channel_flags.premium_subscriptions:
            skib = "**System Boost Messages**: <:tick:1348326381611647046>"
        else:
            skib = "**System Boost Messages**: <:cross:1348326385386651748>"
        ...
        if ctx.guild.mfa_level == discord.MFALevel.disabled:
            mfa = "Enabled <:tick:1348326381611647046>"
        else:
            mfa = "Disabled <:cross:1348326385386651748>"
        embed.add_field(
            name="**__Extras__**",
            value=f"""
...
**2FA Requirement:** {mfa}
**Boost Bar Enabled:** {'<:cross:1348326385386651748>' if not ctx.guild.premium_progress_bar_enabled else '<:tick:1348326381611647046>'}
            """,
            inline=False,
        )
        embed.add_field(
...
{guild.description}
        ...
        if guild.features:
            ...
            embed.add_field(
                name="**__Features__**",
                value="\n".join(
                    [f"<:tick:1348326381611647046> : {feature}" for feature in dk]
                ),
                inline=False,
            )
...
Channels: <:Channel:1263146223364210801> {len(guild.text_channels)} | <:mic:1348326446216515604> {len(guild.voice_channels)} | <:stage:1263165864690192486> {len(guild.stage_channels)} 
{bye}
            """,
                inline=False,
            )
...
Total Emoji: {len(guild.emojis)}/{uf}
        ...
        embed.add_field(
            ...
            value=f"""
Level: {guild.premium_tier} <a:Ignis_dimaond:1263166111512399975> {guild.premium_subscription_count} boosts] 
{byecom}
            """,
            inline=False,
        )
