...
class OnOrOff(BasicView):
    ...
    @discord.ui.button(label="DM me", emoji="<:tick:1348326381611647046>", custom_id='Yes', style=discord.ButtonStyle.green)
    async def dare(self, interaction: discord.Interaction, button: discord.ui.Button):
        self.value = 'Yes'
        await interaction.response.defer()
        self.stop()

    @discord.ui.button(label="Don't DM", emoji="<:cross:1348326385386651748>", custom_id='No', style=discord.ButtonStyle.danger)
...
class afk(commands.Cog):
    ...
    async def afk(self, ctx, *, reason=None):
        ...
        if any(invite in reason.lower() for invite in ['discord.gg', 'gg/']):
            emd = discord.Embed(description="<:alert:1348340453803687966> | You can't advertise Serve Invite in the AFK reason", color=0x0c0606)
        ...
        async with aiosqlite.connect(DB_PATH) as db:
            ...
            await test.delete()
            af = discord.Embed(title='<:tick:1514254173183672381> Success', 
