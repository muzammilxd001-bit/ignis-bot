...
class AutoReaction(commands.Cog):
        ...
        if ctx.invoked_subcommand is None:
            embed = discord.Embed(
                ...
                description=(
                    ...
                    "`react add <trigger> <emojis>` - Add a trigger.\n"
                    "`react remove <trigger>` - Remove a trigger.\n"
                    "`react list` - Show all triggers.\n"
                    "`react reset` - Reset all triggers."
                ),
                color=0x2b2d31
            )
    ...
    async def add(self, ctx, trigger: str, *, emojis: str):
        if len(trigger.split()) > 1:
            embed = discord.Embed(
                title="<:cross:1348326385386651748> Invalid Trigger",
        ...
        if len(emoji_list) > 10:
            embed = discord.Embed(
                title="<:cross:1348326385386651748> Too Many Emojis",
        ...
        if len(triggers) >= 10:
            embed = discord.Embed(
                title="<:alert:1348340453803687966> Trigger Limit Reached",
        ...
        if await self.trigger_exists(ctx.guild.id, trigger):
            embed = discord.Embed(
                title="<:alert:1348340453803687966> Trigger Exists",
        ...
        embed = discord.Embed(
            title="<:tick:1348326381611647046> Trigger Added",
    ...
    async def remove(self, ctx, trigger: str):
        if not await self.trigger_exists(ctx.guild.id, trigger):
            embed = discord.Embed(
                title="<:cross:1348326385386651748> Trigger Not Found",
        ...
        embed = discord.Embed(
            title="<:tick:1348326381611647046> Trigger Removed",
    ...
    async def reset(self, ctx):
        ...
        if not triggers:
            embed = discord.Embed(
                title="<:cross:1348326385386651748> No Triggers Set",
        ...
        embed = discord.Embed(
            title="<:tick:1348326381611647046> All Triggers Reset",
