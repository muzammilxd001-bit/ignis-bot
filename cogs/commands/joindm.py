...
class joindm(commands.Cog):
    ...
    def load_joindm_messages(self):
        ...
        try:
            with open('data/joindm_messages.json', 'r') as f:
    ...
    def save_joindm_messages(self):
        ...
        with open('data/joindm_messages.json', 'w') as f:
    ...
    async def joindm(self, ctx):
        ...
        if guild_id in self.joindm_messages:
            await ctx.send(f"**<:tick:1348326381611647046> |The current join DM message is: `{self.joindm_messages[guild_id]}`")
        else:
            await ctx.send("**<:cross:1348326385386651748> |No custom join DM message has been set for this server. **")
    ...
    async def message(self, ctx, *, message=None):
        ...
        if message is None:
            await ctx.send("** <:cross:1348326385386651748> |Please provide a custom join DM message.**")
        else:
            self.joindm_messages[str(ctx.guild.id)] = message
            self.save_joindm_messages()
            await ctx.send("** <:tick:1348326381611647046> |Custom join DM message set successfully.**")
    ...
    async def enable(self, ctx):
        ...
        await ctx.send("**<:tick:1348326381611647046> |Join DM module enabled. Custom DM will be sent to new members.**")
    ...
    async def disable(self, ctx):
        ...
        await ctx.send("**<:tick:1348326381611647046> |Join DM module disabled. Custom DM will not be sent to new members.**")
    ...
    async def test(self, ctx):
        ...
        if guild_id in self.joindm_messages:
            ...
            await ctx.send("<a:hello:1348887579361280061> Test Sent To Your Dm")
