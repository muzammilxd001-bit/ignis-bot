        ...
        if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
            if streamables.search(thumbnail_link):
                ...
                ray = discord.Embed(
                    color=0x2b2d31,
                    description=
                    "<:tick:1348326381611647046> | Successfully updated the boost thumbnail url .",
                    timestamp=ctx.message.created_at)
                ray.set_author(name=f"{ctx.author.name}",
                                  icon_url=f"{ctx.author.avatar}")
                await ctx.send(embed=ray)
            else:
                await ctx.send("Oops, Kindly put a valid link.")
        else:
            ray5 = discord.Embed(description="""```diff
        ...
        if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
            if streamables.search(image_link):
                ...
                ray = discord.Embed(
                    color=0x2b2d31,
                    description=
                    "<:tick:1348326381611647046> | Successfully updated the boost image url .",
                    timestamp=ctx.message.created_at)
                ray.set_author(name=f"{ctx.author.name}",
                                  icon_url=f"{ctx.author.avatar}")
                await ctx.send(embed=ray)
    ...
    async def _boost_autodel(self, ctx, *, autodelete_second):
        ...
        if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
            ...
            ray = discord.Embed(
                color=0x2b2d31,
                description=
                f"<:tick:1348326381611647046> | Successfully updated the boost autodelete second to {autodelete_second} .\nFrom now boost message will be deleted after {autodelete_second} .",
                timestamp=ctx.message.created_at)
            ray.set_author(name=f"{ctx.author.name}",
                              icon_url=f"{ctx.author.avatar}")
            await ctx.send(embed=ray)
        ...
        if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
            ...
            try:
                ...
                ray = discord.Embed(
                    color=0x2b2d31,
                    description=
                    f"<:tick:1348326381611647046> | Successfully updated the boost message .",
                    timestamp=ctx.message.created_at)
                ray.set_author(name=f"{ctx.author.name}",
                                  icon_url=f"{ctx.author.avatar}")
                await ctx.send(embed=ray)
        ...
        if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
            if data["boost"]["embed"] == True:
                ...
                ray = discord.Embed(
                    color=0x2b2d31,
                    description=
                    f"<:tick:1348326381611647046> | Okay, Now your embed is removed and boost message will be a plain message .",
                    timestamp=ctx.message.created_at)
                ray.set_author(name=f"{ctx.author.name}",
                                  icon_url=f"{ctx.author.avatar}")
                await ctx.send(embed=ray)
                ...
                ray1 = discord.Embed(
                    color=0x2b2d31,
                    description=
                    f"<:tick:1348326381611647046> | Okay, Now your embed is enabled and boost message will be a embed message.",
                    timestamp=ctx.message.created_at)
                ray1.set_author(name=f"{ctx.author.name}",
                                   icon_url=f"{ctx.author.avatar}")
        ...
        if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
            if data["boost"]["ping"] == True:
                ...
                ray = discord.Embed(
                    color=0x2b2d31,
                    description=
                    f"<:tick:1348326381611647046> | Okay, Now your embed ping is disabled and users won't get pinged upon boost .",
                    timestamp=ctx.message.created_at)
                ray.set_author(name=f"{ctx.author.name}",
                                  icon_url=f"{ctx.author.avatar}")
                await ctx.send(embed=ray)
                ...
                ray1 = discord.Embed(
                    color=0x2b2d31,
                    description=
                    f"<:tick:1348326381611647046> | Okay, Now your embed ping is enabled and I will ping new users outside the embed .",
                    timestamp=ctx.message.created_at)
                ray1.set_author(name=f"{ctx.author.name}",
                                   icon_url=f"{ctx.author.avatar}")
                await ctx.send(embed=ray1)
        ...
        if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
            if len(chh) == 1:
                ray = discord.Embed(
                    color=0x2b2d31,
                    description=
                    f"<:cross:1348326385386651748> | You have reached maximum channel limit for channel which is one .",
                    timestamp=ctx.message.created_at)
                ray.set_author(name=f"{ctx.author.name}",
                                  icon_url=f"{ctx.author.avatar}")
                await ctx.send(embed=ray)
                ...
                if str(channel.id) in chh:
                    ray1 = discord.Embed(
                        color=0x2b2d31,
                        description=
                        f"<:cross:1348326385386651748> | This channel is already in the boost channels list .",
                        timestamp=ctx.message.created_at)
                    ray1.set_author(name=f"{ctx.author.name}",
                                       icon_url=f"{ctx.author.avatar}")
                    await ctx.send(embed=ray1)
                    ...
                    ray4 = discord.Embed(
                        color=0x2b2d31,
                        description=
                        f"<:tick:1348326381611647046> | Successfully added {channel.mention} to boost channel list .",
                        timestamp=ctx.message.created_at)
                    ray4.set_author(name=f"{ctx.author.name}",
                                       icon_url=f"{ctx.author.avatar}")
        ...
        if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
            if len(chh) == 0:
                ray = discord.Embed(
                    color=0x2b2d31,
                    description=
                    f"<:cross:1348326385386651748> | This server dont have any boost channel setupped yet .",
                    timestamp=ctx.message.created_at)
                ray.set_author(name=f"{ctx.author.name}",
                                  icon_url=f"{ctx.author.avatar}")
                await ctx.send(embed=ray)
                ...
                if str(channel.id) not in chh:
                    ray1 = discord.Embed(
                        color=0x2b2d31,
                        description=
                        f"<:cross:1348326385386651748> | This channel is not in the boost channels list .",
                        timestamp=ctx.message.created_at)
                    ray1.set_author(name=f"{ctx.author.name}",
                                       icon_url=f"{ctx.author.avatar}")
                    await ctx.send(embed=ray1)
                    ...
                    ray3 = discord.Embed(
                        color=0x2b2d31,
                        description=
                        f"<:tick:1348326381611647046> | Successfully removed {channel.mention} from boost channel list .",
                        timestamp=ctx.message.created_at)
                    ray3.set_author(name=f"{ctx.author.name}",
                                       icon_url=f"{ctx.author.avatar}")
    ...
    async def welctestt(self, ctx):
        ...
        if chan == []:
            ray = discord.Embed(
                color=0x2b2d31,
                description=
                f"<:cross:1348326385386651748> | Oops, Kindly setup your boost channel first .",
                timestamp=ctx.message.created_at)
            ray.set_author(name=f"{ctx.author.name}",
                              icon_url=f"{ctx.author.avatar}")
            await ctx.send(embed=ray)
            ...
            if emautodel == 0 or emautodel == "":
                ...
                emautodel = emautodel
            em = discord.Embed(description=msg, color=0x2b2d31)
    ...
    async def _reset(self, ctx):
        ...
        if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
            if data["boost"]["channel"] == []:
                embed = discord.Embed(
                    description=
                    "<:cross:1348326385386651748> | This server don't have any boost channel setuped yet .",
                    color=0x2b2d31)
                await ctx.send(embed=embed)
            else:
                data["boost"]["channel"] = []
                ...
                ray = discord.Embed(
                    description=
                    "<:tick:1348326381611647046> | Succesfully cleared all boost config for this server .",
                    color=0x2b2d31)
                await ctx.send(embed=ray)
        else:
            ray5 = discord.Embed(
    ...
    async def _boost1_role_reset(self, ctx):
        ...
        if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
            if rl == []:
                embed = discord.Embed(
                    description=
                    "<:cross:1348326385386651748> | This server don't have any boost role setupped .",
                    color=0x2b2d31)
                await ctx.send(embed=embed)
            else:
                if rl != []:
                    ...
                    ray = discord.Embed(
                        description=
                        f"<:tick:1348326381611647046> | Succesfully cleared all boost role for {ctx.guild.name} .",
                        color=0x2b2d31)
                    await ctx.send(embed=ray)
        else:
            ray5 = discord.Embed(
    ...
    async def _boost1_role_add(self, ctx, role: discord.Role):
        ...
        if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
            if len(rl) == 5:
                embed = discord.Embed(
                    description=
                    f"<:cross:1348326385386651748> | You have reached maximum channel limit for boost role which is five .",
                    color=0x2b2d31)
                await ctx.send(embed=embed)
            else:
                if str(role.id) in rl:
                    embed1 = discord.Embed(
                        description=
                        "<:cross:1348326385386651748> | {} is already in boost role ."
                        .format(role.mention),
                        color=0x2b2d31)
                    ...
                    ray = discord.Embed(
                        description=
                        f"<:tick:1348326381611647046> | {role.mention} has been added to boost role .",
                        color=0x2b2d31)
                    await ctx.send(embed=ray)
        else:
            ray5 = discord.Embed(
    ...
    async def _boost1_role_remove(self, ctx, role: discord.Role):
        ...
        if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
            if len(rl) == 0:
                embed = discord.Embed(
                    description=
                    f"<:cross:1348326385386651748> | This server dont have any boost role setupped yet .",
                    color=0x2b2d31)
                await ctx.send(embed=embed)
            else:
                if str(role.id) not in rl:
                    ...
                    ray = discord.Embed(
                        description=
                        f"<:tick:1348326381611647046> | {role.mention} has been removed from boost role .",
                        color=0x2b2d31)
                    await ctx.send(embed=ray)
        else:
            ray5 = discord.Embed(
