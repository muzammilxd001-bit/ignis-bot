import discord
import wavelink
from wavelink.ext import spotify
from discord.ext import commands
import logging
from typing import Any, Dict, Union, Optional
from discord.enums import try_enum
import os
import datetime
import datetime as dt
import datetime
from discord.ui import Button
import random
import typing as t
import requests
import re
from discord.ext.commands.errors import CheckFailure
import asyncio
import os
from wavelink import Player
import async_timeout
from utils.Tools import *
import requests

LYRICS_URL = "https://some-random-api.ml/lyrics?title="
TIME_REGEX = r"([0-9]{1,2})[:ms](([0-9]{1,2})s?)?"
# youtube api key : AIzaSyATb5K-giWsw-72BXFgTNgVo1wV4-sBid0


class NotConnectedToVoice(CheckFailure):
    """User not connected to any voice channel"""

    pass


class PlayerNotConnected(CheckFailure):
    """Player not connected"""

    pass


class MustBeSameChannel(CheckFailure):
    """Player and user not in same channel"""

    pass


class NothingIsPlaying(CheckFailure):
    """Nothing is playing"""

    pass


class NotEnoughSong(CheckFailure):
    """Not enough songs in queue"""

    pass


class InvalidLoopMode(CheckFailure):
    """Invalid loop mode"""

    pass


class DisPlayer(Player):

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)

        self.queue = asyncio.Queue()
        self.bound_channel = None
        self.track_provider = "yt"

    async def destroy(self) -> None:
        self.queue = None

        await super().stop()
        await super().disconnect()

    async def do_next(self) -> None:
        if self.is_playing():
            return

        timeout = int(os.getenv("Ignis_TIMEOUT", 300))

        try:
            with async_timeout.timeout(timeout):
                track = await self.queue.get()
        except asyncio.TimeoutError:
            if not self.is_playing():
                await self.destroy()

            return

        self._source = track
        await self.play(track)
        self.client.dispatch("dismusic_track_start", self, track)
        await self.invoke_player()

    async def invoke_player(self, ctx: commands.Context = None) -> None:
        track = self.source

        if not track:
            raise NothingIsPlaying("Player is not playing anything.")

        embed = discord.Embed(title=track.title, url=track.uri, color=0x2B2D31)
        embed.timestamp = discord.utils.utcnow()
        embed.set_author(
            name=track.author,
            url=track.uri,
            icon_url=self.client.user.display_avatar.url,
        )
        try:
            embed.set_thumbnail(url=track.thumb)
        except AttributeError:
            embed.set_thumbnail(
                url="https://media.discordapp.net/attachments/1349834275410939904/1355756769292320923/ignis.png"
            )

        embed.add_field(
            name="Length",
            value=f"{int(track.length // 60)}:{int(track.length % 60)}",
        )
        embed.add_field(name="Looping", value=self.loop)
        embed.add_field(name="Volume", value=self.volume)

        next_song = ""

        if self.loop == "CURRENT":
            next_song = self.source.title
        else:
            if len(self.queue._queue) > 0:
                next_song = self.queue._queue[0].title

        if next_song:
            embed.add_field(name="Next Song", value=next_song, inline=False)

        if not ctx:
            return await self.bound_channel.send(embed=embed)

        await ctx.send(embed=embed)


class Check:

    async def userInVoiceChannel(self, ctx, bot):
        """Check if the user is in a voice channel"""
        if ctx.author.voice:
            return True
        ray5 = discord.Embed(
            title="Ignis",
            description=f"<:cross:1348326385386651748> {ctx.author.mention} You are not connected in a voice channel",
            color=0x2B2D31,
        )
        ray5.set_footer(
            text=f"Requested By {ctx.author}", icon_url=ctx.author.display_avatar.url
        )
        ray5.set_thumbnail(url=ctx.author.display_avatar.url)
        ray5.timestamp = discord.utils.utcnow()
        await ctx.channel.send(embed=ray5)
        return False

    async def botInVoiceChannel(self, ctx, bot):
        """Check if the bot is in a voice channel"""
        node = wavelink.NodePool.get_node()
        player = node.get_player(ctx.guild)

        if player.is_connected:
            return True
        ray5 = discord.Embed(
            title="Ignis",
            description=f"<:cross:1348326385386651748> {ctx.author.mention} I'm not connected in a voice channel",
            color=0x2B2D31,
        )
        ray5.set_footer(
            text=f"Requested By {ctx.author}", icon_url=ctx.author.display_avatar.url
        )
        ray5.set_thumbnail(url=ctx.author.display_avatar.url)
        ray5.timestamp = discord.utils.utcnow()
        await ctx.channel.send(embed=ray5)
        return False

    async def botNotInVoiceChannel(self, ctx, bot):
        """Check if the bot is not in a voice channel"""
        node = wavelink.NodePool.get_node()
        player = node.get_player(ctx.guild)

        if not player.is_connected:
            return True
        ray5 = discord.Embed(
            title="Ignis",
            description=f"<:cross:1348326385386651748> I'm already connected in a voice channel",
            color=0x2B2D31,
        )
        ray5.set_footer(
            text=f"Requested By {ctx.author}", icon_url=ctx.author.display_avatar.url
        )
        ray5.set_thumbnail(url=ctx.author.display_avatar.url)
        ray5.timestamp = discord.utils.utcnow()
        await ctx.channel.send(embed=ray5)
        return False

    async def userAndBotInSameVoiceChannel(self, ctx, bot):
        """Check if the user and the bot are in the same voice channel"""
        node = wavelink.NodePool.get_node()
        player = node.get_player(ctx.guild)

        if (bot.user.id in ctx.author.voice.channel.voice_states) and (
            ctx.author.id in ctx.author.voice.channel.voice_states
        ):
            return True
        ray5 = discord.Embed(
            title="Ignis",
            description=f"<:cross:1348326385386651748> You are not connected in the same voice channel that the bot",
            color=0x2B2D31,
        )
        ray5.set_footer(
            text=f"Requested By {ctx.author}", icon_url=ctx.author.display_avatar.url
        )
        ray5.set_thumbnail(url=ctx.author.display_avatar.url)
        ray5.timestamp = discord.utils.utcnow()
        await ctx.channel.send(embed=ray5)
        return False

    async def botIsPlaying(self, ctx, bot):
        """Check if the bot is playing"""
        node = wavelink.NodePool.get_node()
        player = node.get_player(ctx.guild)
        if player.is_playing:
            return True
        ray5 = discord.Embed(
            title="Ignis",
            description=f"<:cross:1348326385386651748> There is currently no song to replay",
            color=0x2B2D31,
        )
        ray5.set_footer(
            text=f"Requested By {ctx.author}", icon_url=ctx.author.display_avatar.url
        )
        ray5.set_thumbnail(url=ctx.author.display_avatar.url)
        ray5.timestamp = discord.utils.utcnow()
        await ctx.channel.send(embed=ray5)
        return False


__all__ = (
    "WavelinkError",
    "AuthorizationFailure",
    "LavalinkException",
    "LoadTrackError",
    "BuildTrackError",
    "NodeOccupied",
    "InvalidIDProvided",
    "ZeroConnectedNodes",
    "NoMatchingNode",
    "QueueException",
    "QueueFull",
    "QueueEmpty",
)


class WavelinkError(Exception):
    """Base WaveLink Exception"""


class InvalidEqPreset(commands.CommandError):
    pass


class AuthorizationFailure(WavelinkError):
    """Exception raised when an invalid password is provided toa node."""


class LavalinkException(WavelinkError):
    """Exception raised when an error occurs talking to Lavalink."""


class LoadTrackError(LavalinkException):
    """Exception raised when an error occurred when loading a track."""


class NoLyricsFound(commands.CommandError):
    pass


class NoMoreTracks(commands.CommandError):
    pass


class BuildTrackError(LavalinkException):
    """Exception raised when a track is failed to be decoded and re-built."""

    def __init__(self, data):
        super().__init__(data["error"])


class NodeOccupied(WavelinkError):
    """Exception raised when node identifiers conflict."""


class InvalidTimeString(commands.CommandError):
    pass


class InvalidIDProvided(WavelinkError):
    """Exception raised when an invalid ID is passed somewhere in Wavelink."""


class ZeroConnectedNodes(WavelinkError):
    """Exception raised when an operation is attempted with nodes, when there are None connected."""


class InvalidRepeatMode(commands.CommandError):
    pass


class NoMatchingNode(WavelinkError):
    """Exception raised when a Node is attempted to be retrieved with a incorrect identifier."""


class QueueIsEmpty(commands.CommandError):
    """AtLeast Have  Queue"""


class QueueException(WavelinkError):
    """Base WaveLink Queue exception."""

    pass


class QueueFull(QueueException):
    """Exception raised when attempting to add to a full Queue."""

    pass


class QueueEmpty(QueueException):
    """Exception raised when attempting to retrieve from an empty Queue."""

    pass


VoiceChannel = Union[discord.VoiceChannel, discord.StageChannel]

logger: logging.Logger = logging.getLogger(__name__)


class TrackNotFound(commands.CommandError):
    pass


class Buttons(discord.ui.View):

    def __init__(self):
        super().__init__(timeout=None)

    # <:volume_down:1056039813712707654>

    @discord.ui.button(emoji="🔉", style=discord.ButtonStyle.red, row=0)
    async def volume_button(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
        node = wavelink.NodePool.get_node()
        player = node.get_player(interaction.guild)

        if player is None:
            ray = discord.Embed(
                description="<:cross:1348326385386651748> | I am not connected to a voice channel.",
                color=0x2B2D31,
            )
            return await interaction.response.send_message(embed=ray, ephemeral=True)
        if player.is_playing:
            await player.set_volume(50)
            ray1 = discord.Embed(
                description="<:tick:1348326381611647046> | Successfully changed player volume to : `50`",
                color=0x2B2D31,
            )
            return await interaction.response.send_message(embed=ray1, ephemeral=True)
        else:
            ray2 = discord.Embed(
                description=f"<:cross:1348326385386651748> | I am not playing anything.",
                color=0x2B2D31,
            )
            return await interaction.response.send_message(embed=ray2, ephemeral=True)

    @discord.ui.button(
        emoji="<:DoubleArrowLeft:1263146623983161395>",
        style=discord.ButtonStyle.grey,
        row=0,
    )
    async def seek_button(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
        node = wavelink.NodePool.get_node()
        player = node.get_player(interaction.guild)

        if player is None:
            ray = discord.Embed(
                description="<:cross:1348326385386651748> | I am not connected to a voice channel.",
                color=0x2B2D31,
            )
            return await interaction.response.send_message(embed=ray, ephemeral=True)

        if player.is_playing:
            await player.seek(10 * 1000)
            ray1 = discord.Embed(
                description="<:tick:1348326381611647046> | Seeked the current player to `10 seconds` .",
                color=0x2B2D31,
            )
            return await interaction.response.send_message(embed=ray1, ephemeral=True)
        else:
            ray2 = discord.Embed(
                description="<:cross:1348326385386651748> | I am not playing anything.",
                color=0x2B2D31,
            )
            return await interaction.response.send_message(embed=ray2, ephemeral=True)

    @discord.ui.button(
        emoji="<a:Ignis_playing:1201143295657648189>",
        style=discord.ButtonStyle.danger,
        row=0,
    )
    async def stop_button(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
        node = wavelink.NodePool.get_node()
        player = node.get_player(interaction.guild)

        if player is None:
            ray = discord.Embed(
                description="<:cross:1348326385386651748> | I am not connected to a voice channel.",
                color=0x2B2D31,
            )
            return await interaction.response.send_message(embed=ray, ephemeral=True)
        
        elif player.is_paused():
            await player.resume()
            ray1 = discord.Embed(
                description=f"<:tick:1348326381611647046> | Successfully resumed the player .",
                color=0x2B2D31,
            )
            return await interaction.response.send_message(embed=ray1, ephemeral=True)

        elif player.is_playing:
            await player.pause()
            ray1 = discord.Embed(
                description=f"<:tick:1348326381611647046> | Successfully Paused the player.",
                color=0x2B2D31,
            )
            return await interaction.response.send_message(embed=ray1, ephemeral=True)
        
        else:
            ray2 = discord.Embed(
                description=f"<:cross:1348326385386651748> | I am not playing anything.",
                color=0x2B2D31,
            )
            return await interaction.response.send_message(embed=ray2, ephemeral=True)

    @discord.ui.button(
        emoji="<:DoubleArrowRight:1263147096538615868>",
        style=discord.ButtonStyle.grey,
        row=0,
    )
    async def skip_button(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
        node = wavelink.NodePool.get_node()
        player = node.get_player(interaction.guild)

        if player is None:
            ray = discord.Embed(
                description="<:cross:1348326385386651748> | I am not connected to a voice channel.",
                color=0x2B2D31,
            )
            return await interaction.response.send_message(embed=ray, ephemeral=True)

        if player.is_playing:
            await player.stop()
            ray1 = discord.Embed(
                description="<:tick:1348326381611647046> | Successfully Skipped the track .",
                color=0x2B2D31,
            )
            return await interaction.response.send_message(embed=ray1, ephemeral=True)
        else:
            ray2 = discord.Embed(
                description=f"<:cross:1348326385386651748> | I am not playing anything.",
                color=0x2B2D31,
            )
            return await interaction.response.send_message(embed=ray2, ephemeral=True)

    @discord.ui.button(emoji="🔊", style=discord.ButtonStyle.green, row=0)
    async def vol_button(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
        node = wavelink.NodePool.get_node()
        player = node.get_player(interaction.guild)

        if player is None:
            ray = discord.Embed(
                description="<:cross:1348326385386651748> | I am not connected to a voice channel.",
                color=0x2B2D31,
            )
            return await interaction.response.send_message(embed=ray, ephemeral=True)

        if player.is_playing:
            await player.set_volume(100)
            ray1 = discord.Embed(
                description="<:tick:1348326381611647046> | Successfully changed player volume to : `100`",
                color=0x2B2D31,
            )
            return await interaction.response.send_message(embed=ray1, ephemeral=True)
        else:
            ray2 = discord.Embed(
                description=f"<:cross:1348326385386651748> | I am not playing anything.",
                color=0x2B2D31,
            )
            return await interaction.response.send_message(embed=ray2, ephemeral=True)


class Dropdown(discord.ui.Select):

    def __init__(self):
        options = [
            discord.SelectOption(
                label="Slowed",
                description="Enables Slowed Filter",
                emoji="<:musicss:1348340577112035398>",
            ),
            discord.SelectOption(
                label="Chipmunk",
                description="Enables Chipmunk Filter",
                emoji="<:musicss:1348340577112035398>",
            ),
            discord.SelectOption(
                label="Nightcore",
                description="Enables Nightcore Filter",
                emoji="<:musicss:1348340577112035398>",
            ),
            discord.SelectOption(
                label="Lofi",
                description="Enables Lofi Filter",
                emoji="<:musicss:1348340577112035398>",
            ),
            discord.SelectOption(
                label="8D",
                description="Enables 8D Filter",
                emoji="<:musicss:1348340577112035398>",
            ),
            discord.SelectOption(
                label="Karaoke",
                description="Enables Karaoke Filter",
                emoji="<:musicss:1348340577112035398>",
            ),
            discord.SelectOption(
                label="DeepBass",
                description="Enables Deep Bass Filter",
                emoji="<:musicss:1348340577112035398>",
            ),
                        discord.SelectOption(
                label="Reset", description="Clears all Filter", emoji="♻"
            ),
        ]
        super().__init__(
            placeholder="Select Filter", options=options, min_values=1, max_values=1
        )

    async def callback(self, interaction: discord.Interaction):
        selected_filter = self.values[0]
        if selected_filter == "Slowed":
            vc = interaction.guild.voice_client
            await vc.set_filter(
                wavelink.Filter(timescale=wavelink.Timescale(rate=0.9)), seek=False
            )

        elif selected_filter == "Chipmunk":
            vc = interaction.guild.voice_client
            await vc.set_filter(
                wavelink.Filter(timescale=wavelink.Timescale(rate=1.3)), seek=False
            )

        elif selected_filter == "Reset":
            vc = interaction.guild.voice_client
            await vc.set_filter(
                wavelink.Filter(equalizer=wavelink.Equalizer.flat()), seek=False
            )

        elif selected_filter == "Nightcore":
            vc = interaction.guild.voice_client
            await vc.set_filter(
                wavelink.Filter(timescale=wavelink.Timescale(speed=1.25, pitch=1.3)),
                seek=False,
            )

        elif selected_filter == "Lofi":
            vc = interaction.guild.voice_client
            await vc.set_filter(
                wavelink.Filter(timescale=wavelink.Timescale(rate=0.8)), seek=False
            )

        elif selected_filter == "8D":
            vc = interaction.guild.voice_client
            await vc.set_filter(
                wavelink.Filter(rotation=wavelink.Rotation(speed=0.15)), seek=False
            )

        elif selected_filter == "Karaoke":
            vc = interaction.guild.voice_client
            await vc.set_filter(
                wavelink.Filter(
                    karaoke=wavelink.Karaoke(
                        level=0.9, mono_level=0.9, filter_band=220.0, filter_width=110.0
                    )
                ),
                seek=False,
            )

        elif selected_filter == "DeepBass":
            vc = interaction.guild.voice_client
            bands = [
                (0, 0.3),
                (1, 0.2),
                (2, 0.1),
                (3, 0.05),
                (4, -0.05),
                (5, -0.1),
                (6, -0.1),
                (7, -0.1),
                (8, -0.1),
                (9, -0.1),
                (10, -0.1),
                (11, -0.1),
                (12, -0.1),
                (13, -0.1),
                (14, -0.1),
            ]
            await vc.set_filter(
                wavelink.Filter(
                    equalizer=wavelink.Equalizer(name="Deepbass", bands=bands)
                )
            )

        embed = discord.Embed(
            description=f"`{selected_filter}` **filter will be applied soon...**"
        )
        await interaction.response.send_message(embed=embed, ephemeral=True)


class DropdownView(discord.ui.View):

    def __init__(self):
        super().__init__()
        self.add_item(Dropdown())


class Music(commands.Cog):

    def __init__(self, bot: commands.Bot):
        self.bot: commands.Bot = bot
        self.playlist = []
        self.user_timer = {}
        self.user_all_time = {}

    async def create_nodes(self):
        await self.bot.wait_until_ready()
        lavalink_uri = os.environ.get("LAVALINK_URI", "")
        lavalink_password = os.environ.get("LAVALINK_PASSWORD", os.environ.get("LAVALINK_SERVER_PASSWORD", "youshallnotpass"))
        print(f"[Music] Connecting to Lavalink at {lavalink_uri} ...")
        if not lavalink_uri:
            print("[Music] LAVALINK_URI env var not set — music/Lavalink disabled. Set LAVALINK_URI to enable.")
            return
        try:
            node = wavelink.Node(
                uri=lavalink_uri,
                password=lavalink_password,
            )
            spotify_client = spotify.SpotifyClient(
                client_id="ada2f0339dd340df9aa14c76f33f84f3",
                client_secret="9fd6d604c0f343b4a998839f43ef88c7",
            )
            await wavelink.NodePool.connect(
                client=self.bot,
                nodes=[node],
                spotify=spotify_client,
            )
        except Exception as e:
            print(f"[Music] Lavalink connection failed (music commands unavailable): {e}")

    @commands.Cog.listener()
    async def on_ready(self):
        pass
        asyncio.create_task(self.create_nodes())

    @commands.Cog.listener()
    async def on_wavelink_node_ready(self, node: wavelink.Node):
        print(f"Node <{node.id}> is now Ready!")

    async def cog_command_error(self, ctx, error):
        if isinstance(error, commands.CommandInvokeError):
            error = error.original
        await ctx.send(
            embed=discord.Embed(
                description=f"<:cross:1348326385386651748> | Music error: `{error}`\nMake sure you are in a voice channel and Lavalink is online.",
                color=0x2B2D31,
            )
        )

    @commands.command(name="lavalinkinfo", aliases=["llinfo", "llstats"])
    @blacklist_check()    
    @ignore_check()
    async def lavalink_info(self, ctx):
        if not wavelink.NodePool.nodes:
            return await ctx.send("❌ No Lavalink node is currently connected.")

        node = list(wavelink.NodePool.nodes.values())[0]

        stats = node.stats
        if not stats:
            return await ctx.send("⚠️ Lavalink node is not ready yet.")

        # Format uptime into human-readable duration
        uptime_seconds = stats.uptime / 1000
        uptime_str = str(datetime.timedelta(seconds=int(uptime_seconds)))

        embed = discord.Embed(
            title="📊 Lavalink Node Stats",
            color=discord.Color.blurple()
        )
        embed.add_field(name="🔗 Node URI", value=f"`hello`", inline=False)

        embed.set_footer(text=f"Requested by {ctx.author}", icon_url=ctx.author.display_avatar.url)
        await ctx.send(embed=embed)



    @commands.hybrid_command(
        name="play",
        description="Type Your Song Name",
        usage="play <search>",
        aliases=[("p")],
    )
    @blacklist_check()
    @ignore_check()
    async def play(self, ctx: commands.Context, *, search: str):
        await ctx.defer()
        # Check Lavalink node is connected
        if not wavelink.NodePool.nodes:
            return await ctx.send(
                embed=discord.Embed(
                    description="<:cross:1348326385386651748> | Music is currently unavailable — Lavalink is not connected. Please try again later.",
                    color=0x2B2D31,
                )
            )
        if not getattr(ctx.author, "voice", None):
            nv = discord.Embed(
                description="<:cross:1348326385386651748> | You are not connected to a voice channel.",
                color=0x2B2D31,
            )
            await ctx.send(embed=nv)
            return
        if not ctx.voice_client:
            vc: wavelink.Player = await ctx.author.voice.channel.connect(
                cls=wavelink.Player, self_deaf=True
            )
            embed = discord.Embed(
                description=f"Successfully Connected To {ctx.author.voice.channel.mention}",
                color=0x2B2D31,
            )
            embed.timestamp = discord.utils.utcnow()
            # embed.set_thumbnail(url = f"{ctx.author.avatar}")
            await ctx.send(
                f"<:tick:1348326381611647046> | Successfully Connected To {ctx.author.voice.channel.mention}"
            )
        else:
            vc: wavelink.Player = ctx.voice_client
            vc.chanctx = ctx.channel
        if "https://open.spotify.com" in str(search):

            if vc.queue.is_empty and not vc.is_playing():

                track = await spotify.SpotifyTrack.search(
                    query=search, return_first=True
                )

                await vc.play(track)

                mbed = discord.Embed(
                    color=0x2B2D31,
                    description=f"<:search:1263147608935895113> [{track.title}](https://discord.gg/nakhre) | {track.url}",
                )
                # mbed.add_field(name="<a:playing:1045213196207804568> ", value=f"[{track.title}](https://discord.gg/nakhre)")
                #  mbed.add_field(name="<:search:1263147608935895113> Requested By", value=f"{ctx.author.mention}")
                mbed.add_field(
                    name="<:member:1348326398929932388><:member:1348326398929932388>Song By",
                    value=f"`{track.author}`",
                )
                mbed.add_field(
                    name="<:clock:1348340487383158855> Duration",
                    value=f"`❯ { round(track.duration / 60, 2)}`",
                )
                mbed.set_author(name="NOW PLAYING", icon_url=ctx.author.display_avatar.url)

                mbed.set_thumbnail(url=self.bot.user.display_avatar.url)
                mbed.set_footer(
                    text=f"Requested By {ctx.author}", icon_url=ctx.author.display_avatar.url
                )
                mbed.timestamp = discord.utils.utcnow()

                view = discord.ui.View()
                view = Buttons()
                view.add_item(Dropdown())
                await ctx.send(embed=mbed, view=view)

            else:
                track = await spotify.SpotifyTrack.search(
                    query=search, return_first=True
                )
                await vc.queue.put_wait(track)
                embed = discord.Embed(
                    description=f"[{track}](https://discord.gg/nakhre) | {track.url} Added To The Queue",
                    color=0x2B2D31,
                )

                embed.set_author(name="ADDED TO QUEUE", icon_url=ctx.author.display_avatar.url)
                embed.set_thumbnail(url=track.thumb)
                #     embed.timestamp = discord.utils.utcnow()
                await ctx.send(embed=embed)

        elif "youtube.com" in str(search) or "youtu.be" in str(search):

            track1 = await wavelink.YouTubeTrack.search(query=search, return_first=True)

            if not track1:
                return await ctx.send(
                    embed=discord.Embed(
                        description=":cross: | No track found for that YouTube link.",
                        color=0x2B2D31,
                    )
                )

            if vc.queue.is_empty and not vc.is_playing():

                await vc.play(track1)
                mbed = discord.Embed(
                    color=0x2B2D31,
                    description=f"<:search:1263147608935895113> [{track1.title}]({track1.uri})",
                )
                mbed.add_field(name="Song Url", value=search)
                mbed.set_author(name="NOW PLAYING", icon_url=ctx.author.display_avatar.url)

                if hasattr(track1, "thumb"):
                    mbed.set_image(url=track1.thumb)

                mbed.set_footer(
                    text=f"Requested By {ctx.author}", icon_url=ctx.author.display_avatar.url
                )
                mbed.timestamp = discord.utils.utcnow()
                view = Buttons()
                view.add_item(Dropdown())
                await ctx.send(embed=mbed, view=view)

            else:
                await vc.queue.put_wait(track1)
                embed = discord.Embed(
                    description=f"[{track1.title}]({track1.uri}) Added To The Queue | ",
                    color=0x2B2D31,
                )
                embed.set_author(name="ADDED TO QUEUE", icon_url=ctx.author.display_avatar.url)

                if hasattr(track1, "thumb"):
                    embed.set_thumbnail(url=track1.thumb)

                embed.timestamp = discord.utils.utcnow()
                await ctx.reply(embed=embed)

        else:

            if vc.queue.is_empty and not vc.is_playing():
                if "song" not in search or "music" not in search:
                    search += " song"
                track2 = await wavelink.YouTubeTrack.search(
                    query=search, return_first=True
                )

                await vc.play(track2)

                mbed = discord.Embed(
                    color=0x2B2D31,
                    description=f"[{track2.title}](https://discord.gg/hangouts)",
                )
                # mbed.add_field(name="<a:jk_playing:1045213196207804568> ", value=f"[{track2.title}](https://discord.gg/nakhre)")
                # mbed.add_field(name="<:jk_users:1045213273273929738> Requested By", value=f"{ctx.author.mention}")
                mbed.add_field(
                    name="<:member:1348326398929932388> Song By",
                    value=f"`{track2.author}`",
                )
                mbed.add_field(
                    name="<:clock:1348340487383158855> Duration",
                    value=f"`❯ { round(track2.duration / 60, 2)}`",
                )
                mbed.set_image(url=track2.thumb)

                mbed.set_author(name="NOW PLAYING", icon_url=ctx.author.display_avatar.url)
                mbed.set_footer(
                    text=f"Requested By {ctx.author}", icon_url=ctx.author.display_avatar.url
                )
                mbed.timestamp = discord.utils.utcnow()

                view = discord.ui.View()
                view = Buttons()
                view.add_item(Dropdown())
                await ctx.send(embed=mbed, view=view)

            else:
                if "song" not in search or "music" not in search:
                    search += " song"
                track2 = await wavelink.YouTubeTrack.search(
                    query=search, return_first=True
                )
                await vc.queue.put_wait(track2)
                embed = discord.Embed(
                    description=f"[{track2}](https://discord.gg/hangouts) Added To The Queue",
                    color=0x2B2D31,
                )

                embed.set_author(name="ADDED TO QUEUE", icon_url=ctx.author.display_avatar.url)
                embed.set_thumbnail(url=track2.thumb)
                embed.timestamp = discord.utils.utcnow()
                await ctx.send(embed=embed)

    @commands.command(name="loop")
    async def loop(self, ctx):
        vc = ctx.voice_client

        if not vc:
            return await ctx.send("❌ I'm not connected to a voice channel.")
        if not ctx.author.voice or ctx.author.voice.channel != vc.channel:
            return await ctx.send("❌ You need to be in the same voice channel as the bot.")

        guild_id = ctx.guild.id
        current = self.loop_enabled.get(guild_id, False)
        self.loop_enabled[guild_id] = not current

        if self.loop_enabled[guild_id]:
            await ctx.send("🔁 Looping enabled.")
        else:
            await ctx.send("⏹️ Looping disabled.")

    async def on_track_end(self, player, track, reason):
        # Example logic — assumes you're using Wavelink
        if self.loop_enabled.get(player.guild.id):
            await player.play(track)

    @commands.Cog.listener()
    async def on_wavelink_track_end(self, player: wavelink.Player, track, reason):

        if not player.queue.is_empty:
            ctx = player.chanctx
            new_song = player.queue.get()
            await player.play(new_song)

            if hasattr(new_song, "thumb"):
                mbed = discord.Embed(
                    color=0x2B2D31,
                    description=f"[{new_song.title}](https://discord.gg/nakhre)",
                )
                # mbed.add_field(name="<a:jk_playing:1045213196207804568> ", value=f"[{new_song.title}](https://discord.gg/jkop)")
                # mbed.add_field(name="<:jk_users:1045213273273929738> Requested By", value=f"{ctx.author.mention}")
                mbed.add_field(name="Song By", value=f"`{new_song.author}`")
                mbed.add_field(
                    name="Duration", value=f"`❯ {round(new_song.duration / 60, 2)}`"
                )

                mbed.set_author(
                    name="NOW PLAYING", icon_url=self.bot.user.display_avatar.url
                )
                mbed.set_image(url=new_song.thumb)
                # mbed.timestamp = discord.utils.utcnow()
                view = discord.ui.View()
                view = Buttons()
                view.add_item(Dropdown())
                await ctx.send(embed=mbed, view=view)

            else:
                mbed = discord.Embed(
                    color=0x2B2D31,
                    description=f"[{new_song.title}](https://discord.gg/nakhre)",
                )
                # mbed.add_field(name="<a:jk_playing:1045213196207804568> ", value=f"[{new_song.title}](https://discord.gg/jkop)")
                mbed.add_field(name="Requested By", value=f"{ctx.author.mention}")
                mbed.add_field(name="Song By", value=f"`{new_song.author}`")

                mbed.add_field(
                    name="Duration", value=f"`❯ {round(new_song.duration / 60, 2)}`"
                )
                #           mbed.set_image(url=self.img.value)
                mbed.set_author(
                    name="NOW PLAYING", icon_url=self.bot.user.display_avatar.url
                )
                mbed.set_image(url=new_song.thumb)
                # mbed.timestamp = discord.utils.utcnow()
                view = discord.ui.View()
                view = Buttons()
                view.add_item(Dropdown())
                await ctx.send(embed=mbed, view=view)
        else:
            await player.disconnect()

    @commands.command(
        name="connect",
        help="connect to your channel .",
        aliases=["join", "j", "jvc"],
        usage="connect [channel]",
    )
    @blacklist_check()
    @ignore_check()
    @commands.cooldown(1, 5, commands.BucketType.user)
    async def connect(
        self, ctx: commands.Context, *, channel: discord.VoiceChannel = None
    ):
        """Connects to a voice channel."""
        if not getattr(ctx.author, "voice", None):
            nv = discord.Embed(
                description=f"<:cross:1348326385386651748> | You are not connected to a voice channel.",
                color=0x2B2D31,
            )
            await ctx.send(embed=nv)
            return
        if channel is None:
            channel = ctx.author.voice.channel
        elif ctx.voice_client:
            av = discord.Embed(
                description=f"<:cross:1348326385386651748> | I am already connected to a voice channel.",
                color=0x2B2D31,
            )
            await ctx.send(embed=av)
            return
        vc: wavelink.Player = await channel.connect(cls=wavelink.Player, self_deaf=True)
        sc = discord.Embed(
            description=f"<:tick:1348326381611647046> | Successfully connected to {channel.mention}.",
            color=0x2B2D31,
        )
        await ctx.send(embed=sc)
        await self.auto_disconnect(vc, ctx)

    async def auto_disconnect(self, player, ctx):
        await asyncio.sleep(60)  # Wait for 1 minute
        if player.is_playing():
            await asyncio.sleep(60)
            if player.is_playing():
                return
        await player.disconnect()
        await ctx.send("Auto disconnected due to inactivity.")

    @commands.command(
        name="disconnect", usage="disconnect [channel]", aliases=["leave","dc"]
    )
    @blacklist_check()
    @ignore_check()
    async def leave_command(self, ctx: commands.Context):
        node = wavelink.NodePool.get_node()
        player = node.get_player(ctx.guild)

        if ctx.author.voice is None:
            ray = discord.Embed(
                description="<:cross:1348326385386651748> | You are not connected to a voice channel.",
                color=0x2B2D31,
            )

            # ray.set_thumbnail(url = f"{ctx.author.avatar}")

            return await ctx.reply(embed=ray)

        await player.disconnect()
        ray1 = discord.Embed(
            description=f"<:tick:1348326381611647046> | Successfully disconnected from {ctx.author.voice.channel.mention}",
            color=0x2B2D31,
        )

        await ctx.send(embed=ray1)

    @commands.hybrid_command(name="stop", usage="stop",description="Stop The Player")
    @blacklist_check()
    @ignore_check()
    async def stop_command(self, ctx: commands.Context):
        node = wavelink.NodePool.get_node()
        player = node.get_player(ctx.guild)

        if ctx.author.voice is None:
            ray = discord.Embed(
                description="<:cross:1348326385386651748> | You are not connected to a voice channel.",
                color=0x2B2D31,
            )

            return await ctx.reply(embed=ray)

        if player.is_playing:
            player.queue.clear()
            await player.disconnect()
            ray1 = discord.Embed(
                description=f"<:tick:1348326381611647046> | Destroyed the player.",
                color=0x2B2D31,
            )

            await ctx.send(embed=ray1)
        else:
            ray2 = discord.Embed(
                description=f"<:cross:1348326385386651748> | I am not playing anything.",
                color=0x2B2D31,
            )

            # ray2.set_thumbnail(url = f"{ctx.author.avatar}")

            return await ctx.reply(embed=ray2)
        
    
    @commands.hybrid_command(name="destory", usage="dc",description="Destory The Player")
    @blacklist_check()
    @ignore_check()
    async def stop_command(self, ctx: commands.Context):
        node = wavelink.NodePool.get_node()
        player = node.get_player(ctx.guild)

        if ctx.author.voice is None:
            ray = discord.Embed(
                description="<:cross:1348326385386651748> | You are not connected to a voice channel.",
                color=0x2B2D31,
            )

            return await ctx.reply(embed=ray)

        if player.is_playing:
            player.queue.clear()
            await player.stop()
            ray1 = discord.Embed(
                description=f"<:tick:1348326381611647046> | Destroyed the player.",
                color=0x2B2D31,
            )

            await ctx.send(embed=ray1)
        else:
            ray2 = discord.Embed(
                description=f"<:cross:1348326385386651748> | I am not playing anything.",
                color=0x2B2D31,
            )

            # ray2.set_thumbnail(url = f"{ctx.author.avatar}")

            return await ctx.reply(embed=ray2)


    @commands.hybrid_command(name="skip", usage="skip", aliases=[("s")])
    @blacklist_check()
    @ignore_check()
    async def skip_command(self, ctx: commands.Context):
        node = wavelink.NodePool.get_node()
        player = node.get_player(ctx.guild)

        if ctx.author.voice is None:
            ray = discord.Embed(
                description="<:cross:1348326385386651748> | You are not connected to a voice channel.",
                color=0x2B2D31,
            )
            return await ctx.reply(embed=ray)

        if player.is_playing:
            await player.stop()
            ray1 = discord.Embed(
                description=f"<:tick:1348326381611647046> | Successfully Skipped the track .",
                color=0x2B2D31,
            )

            await ctx.send(embed=ray1)
        else:
            ray2 = discord.Embed(
                description=f"<:cross:1348326385386651748> | I am not playing anything.",
                color=0x2B2D31,
            )
            return await ctx.reply(embed=ray2)

    @commands.command(name="pause", usage="pause")
    @blacklist_check()
    @ignore_check()
    async def pause_command(self, ctx: commands.Context):
        node = wavelink.NodePool.get_node()
        player = node.get_player(ctx.guild)

        if ctx.author.voice is None:
            ray = discord.Embed(
                description="<:cross:1348326385386651748> | You are not connected to a voice channel.",
                color=0x2B2D31,
            )
            ray.set_footer(
                text=f"Requested By {ctx.author}", icon_url=ctx.author.display_avatar.url
            )
            ray.set_thumbnail(url = f"{ctx.author.avatar}")

            return await ctx.reply(embed=ray)

        if not player.is_paused():
            if player.is_playing():
                await player.pause()
                ray1 = discord.Embed(
                    description=f"<:tick:1348326381611647046> | Successfully paused the player .",
                    color=0x2B2D31,
                )
                view = discord.ui.View()
                view = Buttons()
                view.add_item(Dropdown())
                await ctx.send(embed=ray1)
            else:
                ray2 = discord.Embed(
                    description=f"<:cross:1348326385386651748> | I am not playing anything.",
                    color=0x2B2D31,
                )
                return await ctx.reply(embed=ray2)
        else:
            ray3 = discord.Embed(
                description=f"<:cross:1348326385386651748> | Player is already paused .",
                color=0x2B2D31,
            )

            return await ctx.reply(embed=ray3)

    @commands.hybrid_command(name="resume", usage="resume")
    @blacklist_check()
    @ignore_check()
    async def resume_command(self, ctx: commands.Context):
        node = wavelink.NodePool.get_node()
        player = node.get_player(ctx.guild)

        if ctx.author.voice is None:
            ray = discord.Embed(
                description="<:cross:1348326385386651748> | You are not connected to a voice channel.",
                color=0x2B2D31,
            )

            return await ctx.reply(embed=ray)

        if player.is_paused():
            await player.resume()
            ray1 = discord.Embed(
                description=f"<:tick:1348326381611647046> | Successfully resumed the player .",
                color=0x2B2D31,
            )

            await ctx.send(embed=ray1)
        else:
            ray3 = discord.Embed(
                description=f"<:cross:1348326385386651748> | Player is already resumed .",
                color=0x2B2D31,
            )

            return await ctx.reply(embed=ray3)

    @commands.group(name="bassboost", invoke_without_command=True, aliases=["bass"])
    @blacklist_check()
    @ignore_check()
    async def _bass(self, ctx):
        if ctx.subcommand_passed is None:
            await ctx.send_help(ctx.command)
            ctx.command.reset_cooldown(ctx)

    @_bass.command(name="enable", aliases=[("on")])
    @blacklist_check()
    @ignore_check()
    async def boost_command(self, ctx: commands.Context):
        vc: wavelink.Player = ctx.voice_client

        if vc is None:
            ray = discord.Embed(
                description="<:cross:1348326385386651748> | You are not connected to a voice channel.",
                color=0x2B2D31,
            )
            ray.set_footer(
                text=f"Requested By {ctx.author}", icon_url=ctx.author.display_avatar.url
            )
            # ray.set_thumbnail(url = f"{ctx.author.avatar}")

            return await ctx.reply(embed=ray)
        bands = [
            (0, 0.2),
            (1, 0.15),
            (2, 0.1),
            (3, 0.05),
            (4, 0.0),
            (5, -0.05),
            (6, -0.1),
            (7, -0.1),
            (8, -0.1),
            (9, -0.1),
            (10, -0.1),
            (11, -0.1),
            (12, -0.1),
            (13, -0.1),
            (14, -0.1),
        ]
        await vc.set_filter(
            wavelink.Filter(
                equalizer=wavelink.Equalizer(name="MyOwnFilter", bands=bands)
            ),
            seek=True,
        )
        ray4 = discord.Embed(
            description="<:tick:1348326381611647046> | Successfully enabled `bass boost` .",
            color=0x2B2D31,
        )

        await ctx.reply(embed=ray4)

    @_bass.command(name="disable", aliases=[("off")])
    @commands.has_permissions(administrator=False)
    @blacklist_check()
    @ignore_check()
    async def rmvboost_command(self, ctx: commands.Context):
        vc: wavelink.Player = ctx.voice_client
        await vc.set_filter(
            wavelink.Filter(equalizer=wavelink.Equalizer.flat()), seek=True
        )
        ray4 = discord.Embed(
            description="<:tick:1348326381611647046> | Successfully disabled `bass boost` .",
            color=0x2B2D31,
        )
        await ctx.reply(embed=ray4)

    @commands.hybrid_command(name="move", usage="move <VoiceChannel>")
    @blacklist_check()
    @ignore_check()
    async def move_to(self, ctx, channel: discord.VoiceChannel) -> None:
        await ctx.guild.change_voice_state(channel=channel)
        ray4 = discord.Embed(
            description=f"Moving to voice channel:: {channel.id} .", color=0x2B2D31
        )

        await ctx.send(embed=ray4)

    @commands.command(name="volume", usage="volume <vol>", aliases=[("vol")])
    @commands.guild_only()
    @blacklist_check()
    @ignore_check()
    @commands.cooldown(1, 5, commands.BucketType.member)
    async def volume(self, ctx, volume):

        if not await Check().userInVoiceChannel(ctx, self.bot):
            return
        if not await Check().botInVoiceChannel(ctx, self.bot):
            return
        if not await Check().userAndBotInSameVoiceChannel(ctx, self.bot):
            return

        if (not volume.isdigit()) or (int(volume)) < 0 or (int(volume) > 250):
            ray = discord.Embed(
                description="<:cross:1348326385386651748> | Volume Must Be 0 To 250 .",
                color=0x2B2D31,
            )
            ray.set_footer(
                text=f"Requested By {ctx.author}", icon_url=ctx.author.display_avatar.url
            )
            ray.set_thumbnail(url = f"{ctx.author.avatar}")

            return await ctx.send(embed=ray)
        node = wavelink.NodePool.get_node()
        player = node.get_player(ctx.guild)
        volume = int(volume)
        await player.set_volume(volume)
        ray4 = discord.Embed(
            description=f"<:tick:1348326381611647046> | Successfully changed player volume to : `{volume}%`",
            color=0x2B2D31,
        )

        await ctx.send(embed=ray4)

    @commands.command(name="nowplaying", usage="nowplaying", aliases=[("now")])
    @blacklist_check()
    @ignore_check()
    async def playing(self, ctx):
        if not ctx.voice_client:
            ray = discord.Embed(
                description="<:cross:1348326385386651748> | Not connected to a voice channel.",
                color=0x2B2D31,
            )
            ray.set_footer(
                text=f"Requested By {ctx.author}", icon_url=ctx.author.display_avatar.url
            )
            ray.set_thumbnail(url=ctx.author.display_avatar.url)

            return await ctx.send(embed=ray)
        elif not getattr(ctx.author.voice, "channel", None):
            ray1 = discord.Embed(
                description="<:cross:1348326385386651748> | You are not connected to a voice channel.",
                color=0x2B2D31,
            )
            ray1.set_footer(
                text=f"Requested By {ctx.author}", icon_url=ctx.author.display_avatar.url
            )
            ray1.set_thumbnail(url=ctx.author.display_avatar.url)
            ray1.timestamp = discord.utils.utcnow()
            return await ctx.send(embed=ray1)
        else:
            vc: wavelink.Player = ctx.voice_client

        if not vc.is_playing():
            ray1 = discord.Embed(
                description="<:cross:1348326385386651748> | I am not playing anything .",
                color=0x2B2D31,
            )
            ray1.set_footer(
                text=f"Requested By {ctx.author}", icon_url=ctx.author.display_avatar.url
            )
            ray1.set_thumbnail(url=ctx.author.display_avatar.url)
            ray1.timestamp = discord.utils.utcnow()
            return await ctx.send(embed=ray1)

        em = discord.Embed(
            description=f"[{vc.track}](https://discord.gg/nakhre)",
            color=0x2B2D31,
        )

        em.add_field(
            name="<:member:1348326398929932388> Song By", value=f"`{vc.track.author}`"
        )
        em.add_field(
            name="<:clock:1348340487383158855> Duration",
            value=f"`❯ {datetime.timedelta(seconds=vc.track.length)}`",
        )
        em.set_footer(
            text=f"Requested By {ctx.author}", icon_url=ctx.author.display_avatar.url
        )
        em.set_author(name="NOW PLAYING", icon_url=ctx.author.display_avatar.url)
        em.set_thumbnail(url=ctx.author.display_avatar.url)
        em.timestamp = discord.utils.utcnow()
        return await ctx.send(embed=em)

    @commands.command(name="shuffle", usage="shuffle", aliases=[("shuff")])
    @blacklist_check()
    @ignore_check()
    async def shuffle(self, ctx):
        if not ctx.voice_client:
            ray = discord.Embed(
                description="<:cross:1348326385386651748> | Not connected to a voice channel.",
                color=0x2B2D31,
            )

            return await ctx.send(embed=ray)
        elif not getattr(ctx.author.voice, "channel", None):
            ray1 = discord.Embed(
                description="<:cross:1348326385386651748> | You are not connected to a voice channel.",
                color=0x2B2D31,
            )

            return await ctx.send(embed=ray1)
        else:
            vc: wavelink.Player = ctx.voice_client

        copy = vc.queue.copy()
        random.shuffle(copy)
        vc.queue = copy
        ray2 = discord.Embed(
            description="<:tick:1348326381611647046> | Successfully shuffled the current queue .",
            color=0x2B2D31,
        )
        await ctx.send(embed=ray2)

    @commands.command(name="pull", usage="pull <index>")
    @blacklist_check()
    @ignore_check()
    async def pull(self, ctx, index: int):
        if not ctx.voice_client:
            ray = discord.Embed(
                description="<:cross:1348326385386651748> | Not connected to a voice channel.",
                color=0x2B2D31,
            )

            return await ctx.send(embed=ray)
        elif not getattr(ctx.author.voice, "channel", None):
            ray1 = discord.Embed(
                description="<:cross:1348326385386651748> | You are not connected to a voice channel.",
                color=0x2B2D31,
            )
            return await ctx.send(embed=ray1)
        else:
            vc: wavelink.Player = ctx.voice_client

        if index > len(vc.queue) or index < 1:
            ray2 = discord.Embed(
                description=f"<:cross:1348326385386651748> | Must Be Between 1 And {len(vc.queue)} .",
                color=0x2B2D31,
            )
            return await ctx.reply(embed=ray2)

        removed = vc.queue.pop(index - 1)
        ray3 = discord.Embed(
            description=f"<:tick:1348326381611647046> | Successfully pulled out `{removed.title}` From Queue .",
            color=0x2B2D31,
        )
        await ctx.send(embed=ray3)

    @commands.group(name="queue", invoke_without_command=True, aliases=["q"])
    @blacklist_check()
    @ignore_check()
    async def _queue(self, ctx):
        if not ctx.voice_client:
            ray = discord.Embed(
                description="<:cross:1348326385386651748> | You are not connected to a voice channel.",
                color=0x2B2D31,
            )

            return await ctx.reply(embed=ray)
        elif not getattr(ctx.author.voice, "channel", None):
            ray1 = discord.Embed(
                description="<:cross:1348326385386651748> | You need to join a voice channel to play something .",
                color=0x2B2D31,
            )
            return await ctx.send(embed=ray1)
        else:
            vc: wavelink.Player = ctx.voice_client

        if vc.queue.is_empty:
            ray3 = discord.Embed(
                description=f"<:cross:1348326385386651748> | No songs in queue .",
                color=0x2B2D31,
            )
            ray3.set_footer(
                text=f"Requested By {ctx.author}", icon_url=ctx.author.display_avatar.url
            )
            ray3.set_thumbnail(url = f"{ctx.author.avatar}")
            ray3.timestamp = discord.utils.utcnow()
            return await ctx.send(embed=ray3)
        ray4 = discord.Embed(title="Music | Queue", color=0x2B2D31)
        ray4.set_footer(
            text=f"Requested By {ctx.author}", icon_url=ctx.author.display_avatar.url
        )
        ray4.set_thumbnail(url=ctx.author.display_avatar.url)

        copy = vc.queue.copy()
        count = 0
        for song in copy:
            count += 1
            ray4.add_field(
                name=f"Position : {count}",
                value=f"[{song.title}](https://discord.gg/nakhre)",
            )
        return await ctx.send(embed=ray4)

    @_queue.command(name="clear", aliases=[("c")])
    @blacklist_check()
    @ignore_check()
    async def _clear(self, ctx):
        if not ctx.voice_client:
            ray = discord.Embed(
                description="<:cross:1348326385386651748> | Not connected to a voice channel.",
                color=0x2B2D31,
            )

            return await ctx.send(embed=ray)
        elif not getattr(ctx.author.voice, "channel", None):
            ray1 = discord.Embed(
                description="<:cross:1348326385386651748> | You are not connected to a voice channel.",
                color=0x2B2D31,
            )

            return await ctx.send(embed=ray1)
        else:
            vc: wavelink.Player = ctx.voice_client

        await vc.queue.clear()
        ray3 = discord.Embed(
            description=f"<:tick:1348326381611647046> | Successfully Clears The current Queue .",
            color=0x2B2D31,
        )
        ray3.set_footer(
            text=f"Requested By {ctx.author}", icon_url=ctx.author.display_avatar.url
        )
        return await ctx.send(embed=ray3)

    @commands.command(name="seek", aliases=["sk"], usage="seek")
    @blacklist_check()
    @ignore_check()
    async def seek_command(self, ctx, position: str):
        node = wavelink.NodePool.get_node()
        player: Player = node.get_player(ctx.guild)

        if not (match := re.match(TIME_REGEX, position)):
            raise InvalidTimeString

        if match.group(3):
            secs = (int(match.group(1)) * 60) + (int(match.group(3)))
        else:
            secs = int(match.group(1))

        await player.seek(secs * 1000)
        ray3 = discord.Embed(
            description=f"<:tick:1348326381611647046> | Successfully Seeked the current player to {secs} .",
            color=0x2B2D31,
        )
        await ctx.reply(embed=ray3)
