...
class PaginationViewWallah:
  ...
  async def start(self, ctx, disxd=False):
    ...
    menu.add_button(bax)
    bax2 = ViewButton(style=discord.ButtonStyle.danger, label=None,emoji='<:cross:1348326385386651748>', custom_id=ViewButton.ID_END_SESSION)
...
class owner1(Cog):
  ...
  def __init__(self, client: Ignis):
    self.client = client
    self.owner_ids = [870179991462236170, 912422872411160667, 806624430322483220] # Ignis PREMIUM User
  ...
  async def np_add(self, ctx: Context, user: discord.User):
    if ctx.message.author.id not in self.owner_ids:
            await ctx.send("<:cross:1348326385386651748> | this command can only be executed by my developers .")
            return
    with open('info.json', 'r') as idk:
      data = json.load(idk)
    np = data["np"]
    if user.id in np:
      await ctx.send(embed=discord.Embed(description=f"<:cross:1348326385386651748> | {user.name} Already have no prefix",color=0x2b2d31))
    else:
      data["np"].append(user.id)
    with open('info.json', 'w') as idk:
      json.dump(data, idk, indent=4)
      await ctx.send(embed=discord.Embed(description=f"<:tick:1348326381611647046> | Added no prefix to {user.name}",color=0x2b2d31))
  ...
  async def np_remove(self, ctx: Context, user: discord.User):
    ...
    if user.id not in np:
      await ctx.send(embed=discord.Embed(description=f"<:cross:1348326385386651748> | {user.name} Doesn't have no prefix",color=0x2b2d31))
    else:
      data["np"].remove(user.id)
    with open('info.json', 'w') as idk:
      json.dump(data, idk, indent=4)
      await ctx.send(embed=discord.Embed(description=f"<:tick:1348326381611647046> | Removed no prefix from {user.name}",color=0x2b2d31))
  ...
  async def bye_buddy(self, ctx):
    ...
    with open("info.json", "r") as f:
      file = json.load(f)
    if file["np"] == []:
      return await ctx.send("<:cross:1348326385386651748> | No one have no prefix.")
    comrade = []
    for idk in file["np"]:
      member = await self.client.fetch_user(int(idk))
      comrade.append(member)
    await working_lister(title="List Of Users In No Prefix", ctx=ctx, color=0x2b2d31, listxd=comrade)
