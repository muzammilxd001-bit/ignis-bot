...
class NotifCommands(commands.Cog):
    ...
    async def setnotify(self, ctx):
        embed = discord.Embed(title="Notification Commands", color=0x2b2d31)
    ...
    async def twitch(self, ctx, role: discord.Role, channel: discord.TextChannel):
        async with aiosqlite.connect(self.db_path) as db:
            async with db.execute('SELECT * FROM notifications WHERE type = ?', ('twitch',)) as existing:
                ...
                if row:
                    await ctx.reply(embed=discord.Embed(title="<:cross:1348326385386651748> Access Denied", description="Twitch notification already set. Remove it first.", color=0x2b2d31))
                    return

            await db.execute('INSERT INTO notifications (type, role_id, channel_id) VALUES (?, ?, ?)', ('twitch', role.id, channel.id))
            await db.commit()
            await ctx.reply(embed=discord.Embed(title="<:tick:1348326381611647046> Success", description=f"Twitch notifications set for {role.mention} in {channel.mention}.", color=0x2b2d31))
    ...
    async def youtube(self, ctx, role: discord.Role, channel: discord.TextChannel):
        async with aiosqlite.connect(self.db_path) as db:
            async with db.execute('SELECT * FROM notifications WHERE type = ?', ('youtube',)) as existing:
                row = await existing.fetchone()
                if row:
                    await ctx.reply(embed=discord.Embed(title="<:cross:1348326385386651748> Access Denied", description="YouTube notification already set. Remove it first.", color=0x2b2d31))
                    return

            await db.execute('INSERT INTO notifications (type, role_id, channel_id) VALUES (?, ?, ?)', ('youtube', role.id, channel.id))
            await db.commit()
            await ctx.reply(embed=discord.Embed(title="<:tick:1348326381611647046> Success", description=f"YouTube notifications set for {role.mention} in {channel.mention}.", color=0x2b2d31))
    ...
    async def list(self, ctx):
        async with aiosqlite.connect(self.db_path) as db:
            ...
            embed = discord.Embed(title="Current Notification Settings", color=0x2b2d31)
            for row in rows:
    ...
    async def reset(self, ctx):
        async with aiosqlite.connect(self.db_path) as db:
            ...
            await db.commit()
            await ctx.send(embed=discord.Embed(title="<:tick:1348326381611647046> Success", description="Twitch and YouTube notifications have been reset.", color=0x00FF00))
