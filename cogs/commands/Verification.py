...
class Verification(commands.Cog):
  ...
  async def on_interaction(self, interaction):
    with open("data/verification.json", "r") as f:
      data = json.load(f)
    if interaction.message is not None and str(interaction.message.guild.id) not in data:
      return
    elif interaction.data.get("custom_id") != "Verify":
      return
    else:
      if interaction.message.guild.get_role(int(data[str(interaction.message.guild.id)]["role"])) in interaction.user.roles:
        ...
        try:
          ...
          await interaction.response.send_message(content=f"<:cross:1348326385386651748> Oops! There was something wrong while adding role. | {e}", ephemeral=True)
  ...
  async def verify_enable(self, ctx, channel: discord.TextChannel, role: discord.Role):
    ...
    if str(ctx.guild.id) in data:
      ...
      btn = Button(style=discord.ButtonStyle.grey, label="", emoji="<:tick:1348326381611647046>", custom_id="Verify")
      view = View()
      view.add_item(btn)
      embed = discord.Embed(title='Verification', description=f"To Access In **{ctx.guild.name}** Press the <:tick:1348326381611647046> Button Below", color=0x2b2d31)
      lame = await channel.send(embed=embed, view=view)
      await idk.delete()
      await ctx.send("<:tick:1348326381611647046> | Successfully Completed Verification Setup!")
  ...
  async def verify_disable(self, ctx):
    ...
    if str(ctx.guild.id) not in data:
      ...
      with open("data/verification.json", "w") as f:
        json.dump(data, f, indent=4)
      await ctx.send("<:tick:1348326381611647046> | Successfully Deleted Verification System In This Guild!")
  ...
  async def verify_config(self, ctx):
    with open("data/verification.json", "r") as f:
      data = json.load(f)
    embed = discord.Embed(title="Verification Configuration", color=0x2b2d31)
