...
class Moderation(commands.Cog):
    ...
    @commands.command(
        name="unlockall", help="Unlocks down the server.", usage="unlockall"
    )
    @blacklist_check()
    @ignore_check()
    ...
    async def unlockall(self, ctx, server: discord.Guild = None, *, reason=None):
        ray = discord.Embed(
            color=0x2B2D31,
            description="<:tick:1348326381611647046> | Unlocking all channels in few seconds .",
            timestamp=ctx.message.created_at,
        )
        ray.set_author(name=f"{ctx.author.name}", icon_url=f"{ctx.author.avatar}")
        ...
        try:
            ...
            embed = discord.Embed(
                color=0x2B2D31, description=f"**Failed to unlock, {server}.**"
            )
    ...
    @commands.command(name="lockall", help="Locks down the server.", usage="lockall")
    @blacklist_check()
    @ignore_check()
    @commands.has_permissions(administrator=True)
    @commands.cooldown(1, 5, commands.BucketType.channel)
    async def lockall(self, ctx, server: discord.Guild = None, *, reason=None):
        ray = discord.Embed(
            color=0x2B2D31,
            description="<:tick:1348326381611647046> | Locking all channels in few seconds .",
            timestamp=ctx.message.created_at,
        )
        ray.set_author(name=f"{ctx.author.name}", icon_url=f"{ctx.author.avatar}")
        await ctx.reply(embed=ray)
        ...
        try:
            ...
            embed = discord.Embed(
                color=0x2B2D31, description=f"**Failed to lockdown, {server}.**"
            )
            embed.set_author(name=f"{ctx.author.name}", icon_url=f"{ctx.author.avatar}")
            await ctx.send(embed=embed)
        else:
    ...
    async def give(
        ...
        if guild.me.top_role >= ctx.author.top_role:
            embed = discord.Embed(
                color=0x2B2D31,
                description="<:cross:1348326385386651748> | Your top role should be above my top role.",
            )
            await ctx.send(embed=embed)
            return
        if role.position >= guild.me.top_role.position:
            await ctx.send(
                f"**<:cross:1348326385386651748> | The position of `{role}` is above my top role. **"
            )
            return
        else:
            ...
            embed.set_footer(
                text=f"Done by {ctx.author}", icon_url=f"{ctx.author.avatar}"
            )
            embed.add_field(
                name="Added Role", value=f"{role} has been given to {member.name}"
            )
            await ctx.send(embed=embed)
    ...
    async def _hideall(self, ctx):
        ray = discord.Embed(
            color=0x2B2D31,
            description="<:tick:1348326381611647046> | Hiding all channels in few seconds .",
            timestamp=ctx.message.created_at,
        )
        ray.set_author(name=f"{ctx.author.name}", icon_url=f"{ctx.author.avatar}")
    ...
    async def _unhideall(self, ctx):
        ray = discord.Embed(
            color=0x2B2D31,
            description=f"<:tick:1348326381611647046> | Unhiding all channels in few seconds .",
            timestamp=ctx.message.created_at,
        )
        ray.set_author(name=f"{ctx.author.name}", icon_url=f"{ctx.author.avatar}")
        # ray.set_thumbnail(url =f"{ctx.author.avatar}")
    ...
    async def _hide(self, ctx, channel: discord.TextChannel = None):
        ...
        ray = discord.Embed(
            color=0x2B2D31,
            description=f"<:tick:1348326381611647046> | Succefully Hidden {channel.mention} .",
            timestamp=ctx.message.created_at,
        )
        ray.set_author(name=f"{ctx.author.name}", icon_url=f"{ctx.author.avatar}")
        ray.set_thumbnail(url=f"{ctx.author.avatar}")
    ...
    async def _unhide(self, ctx, channel: discord.TextChannel = None):
        ...
        ray = discord.Embed(
            color=0x2B2D31,
            description=f"<:tick:1348326381611647046> | Succefully Unhidden {channel.mention} .",
            timestamp=ctx.message.created_at,
        )
        ray.set_author(name=f"{ctx.author.name}", icon_url=f"{ctx.author.avatar}")
        ray.set_thumbnail(url=f"{ctx.author.avatar}")
...
Reason: `{entry.reason}`\n\n"""
        ...
        embed = discord.Embed(
            title=f"Audit Logs Of {ctx.guild.name}",
            description=f">>> {str}",
            color=0x2B2D31,
        )
    ...
    @commands.hybrid_command(
        name="softban",
        help="Literally trolling command or you can use to clear all messages by the user.",
    ...
    async def _softban(
        ...
        ray = discord.Embed(
            color=0x2B2D31,
            description=f"<:tick:1348326381611647046> | Sucessfully soft-banned {member}.",
            timestamp=ctx.message.created_at,
        )
        ray.set_footer(
            text=f"Made With 💖 By ~ Ray",
    ...
    @commands.hybrid_command(
        name="unmute", description="Unmutes a member .", usage="unmute <member>"
    )
    @blacklist_check()
    ...
    async def untimeout(self, ctx, member: discord.Member):
        if member.is_timed_out():
            try:
                ...
                ray5 = discord.Embed(
                    color=0x2B2D31,
                    description=f"<:tick:1348326381611647046> | Successfully Unmuted {member.name}",
                    timestamp=ctx.message.created_at,
                )
                await ctx.reply(embed=ray5)
            except Exception as e:
                ray = discord.Embed(
                    color=0x2B2D31,
                    description="<:cross:1348326385386651748> | Unable to Remove Timeout:\n```py\n{}```".format(
                        e
                    ),
                    timestamp=ctx.message.created_at,
                )
            ...
            ray1 = discord.Embed(
                color=0x2B2D31,
                description="<:cross:1348326385386651748> | {} Is Not Muted".format(
                    member.mention
                ),
                timestamp=ctx.message.created_at,
            )
    ...
    async def _kick(
        ...
        if (
            ...
            ray = discord.Embed(
                color=0x2B2D31,
                description=f"<:tick:1348326381611647046> | {member.display_name} has been kicked from this guild, for: {reason}",
                timestamp=ctx.message.created_at,
            )
            ray.set_footer(
                text=f"Made With 💖 By ~ Ray",
            ...
            ray1 = discord.Embed(
                color=0x2B2D31,
                description=f"<:kick:1263145846799728720> | You have been kicked from {ctx.guild.name} for: {reason}!",
                timestamp=ctx.message.created_at,
            )
            await member.send(embed=ray1)
        if (
            not ctx.author.top_role.position > member.top_role.position
            and ctx.author != ctx.guild.owner
        ):
            await ctx.send(
                "*<:cross:1348326385386651748> | You cannot kick someone with a higher role than you!*"
            )

    @commands.hybrid_command(
        name="warn", help="To warn a specific user.", usage="warn <member>"
    )
    @blacklist_check()
    @commands.has_permissions(kick_members=True)
    async def _warn(
        ...
        *,
        ...
        ray = discord.Embed(
            color=0x2B2D31,
            description=f"<:tick:1348326381611647046> | {member.display_name} has been warned for: {reason}",
            timestamp=ctx.message.created_at,
        )
        ray.set_footer(
            text=f"Made With 💖 By ~ Ray",
        ...
        ray1 = discord.Embed(
            color=0x886AD1,
            description=f"<:member:1348326398929932388> | You have been warned in {ctx.guild.name} for: {reason}",
            timestamp=ctx.message.created_at,
        )
        ray1.set_footer(
            text=f"Made With 💖 By ~ Ray",
        ...
        ray1.set_thumbnail(
            url="https://media.discordapp.net/attachments/1349834275410939904/1355756857246879845/ignis1.png"
        )
        await member.send(embed=ray1)

    @commands.hybrid_command(
        name="ban",
        help="Somebody is breaking rules again and again | ban him from the server as punishment",
        aliases=[
            "fuckban",
            "chodaban",
            "vodaban",
    ...
    async def _ban(self, ctx: commands.Context, member: discord.Member, *, reason=None):
        ...
        if (
            ...
            ray = discord.Embed(
                color=0x2B2D31,
                description=f"<:tick:1348326381611647046> | {member.display_name} has been successfully banned for the reason: `{reason}`",
                timestamp=ctx.message.created_at,
            )
            ray.set_author(name=f"{ctx.author.name}", icon_url=f"{ctx.author.avatar}")
            ray.set_thumbnail(url=f"{ctx.author.avatar}")
            ...
            ray1 = discord.Embed(
                color=0x2B2D31,
                description=f"<:kick:1263145846799728720> | You have been banned from {ctx.message.guild.name} for reason: `{reason}`",
                timestamp=ctx.message.created_at,
            )
            await member.send(embed=ray1)
        if (
            ...
            embed = discord.Embed(
                color=0x2B2D31,
                description=f"*<:cross:1348326385386651748> | You cannot ban someone with a higher role than you.*",
                timestamp=ctx.message.created_at,
            )
            await ctx.send(embed=embed)
    ...
    async def _unban(self, ctx: commands.Context, id: int):
        ...
        ray = discord.Embed(
            color=0x2B2D31,
            description=f"<:tick:1348326381611647046> | {user.name} has been successfully unbanned",
            timestamp=ctx.message.created_at,
        )
        ray.set_footer(
            text=f"Made With 💖 By ~ Ray",
    ...
    async def clone(self, ctx: commands.Context, channel: discord.TextChannel):
        ...
        ray = discord.Embed(
            color=0x2B2D31,
            description=f"<:tick:1348326381611647046> | {channel.name} has been successfully cloned",
            timestamp=ctx.message.created_at,
        )
        ray.set_footer(
            text=f"Made With 💖 By ~ Ray",
    ...
    async def changenickname(
        self, ctx: commands.Context, member: discord.Member, *, nick
    ):
        await member.edit(nick=nick)
        ray = discord.Embed(
            color=0x2B2D31,
            description=f"<:tick:1348326381611647046> | Successfully changed nickname of {member.name}",
            timestamp=ctx.message.created_at,
        )
        ray.set_author(name=f"{ctx.author.name}", icon_url=f"{ctx.author.avatar}")
        ray.set_thumbnail(url=f"{ctx.author.avatar}")
            ...
            if interaction.user == ctx.author:
                if interaction.guild.me.guild_permissions.manage_channels:
                    ...
                    await interaction.response.edit_message(
                        content="I am missing manage channels permission.\ntry giving me permissions and retry",
                        embed=None,
                        view=None,
                    )
            else:
                await interaction.response.send_message(
                    "This Is Not For You Dummy!", embed=None, view=None, ephemeral=True
                )
    ...
    @commands.has_permissions(manage_channels=True)
    async def _lock(
        self, ctx: commands.Context, channel: discord.TextChannel = None, *, reason=None
    ):
        if channel is None:
            channel = ctx.channel
        try:
            ...
            await ctx.send(
                embed=discord.Embed(
                    title="Ignis | Lockdown",
                    description="<:tick:1348326381611647046> | Successfully locked **%s**"
                    % (channel.mention),
                    color=0x2B2D31,
                )
            )
            ...
            await ctx.send(
                embed=discord.Embed(
                    title="Ignis | Lockdown",
                    description="Failed to lockdown **%s**" % (channel.mention),
                    color=0x2B2D31,
                )
            )
        else:
    ...
    async def _unlock(
        ...
        try:
            ...
            await ctx.send(
                embed=discord.Embed(
                    title="Ignis | Unlockdown",
                    description="<:tick:1348326381611647046> | Successfully unlocked **%s**"
                    % (channel.mention),
                    color=0x2B2D31,
                )
            )
            ...
            await ctx.send(
                embed=discord.Embed(
                    title="Ignis | Unlockdown",
                    description="Failed to lock **`%s`**" % (channel.mention),
                    color=0x2B2D31,
                )
            )
        else:
    ...
    async def _slowmode(self, ctx: commands.Context, seconds: int = 0):
        ...
        if seconds == 0:
            ...
            await ctx.send(
                embed=discord.Embed(
                    title="slowmode", description="Slowmode is disabled", color=0x2B2D31
                )
            )
        else:
            await ctx.channel.edit(slowmode_delay=seconds)
    ...
    async def _unslowmode(self, ctx: commands.Context):
        ...
        await ctx.send(
            embed=discord.Embed(
                title="unslowmode", description="Disabled slowmode", color=0x2B2D31
            )
        )

    @commands.hybrid_command(
        help="Search for emojis!", aliases=["searchemoji", "findemoji", "emojifind"]
    )
    @blacklist_check()
    @ignore_check()
    @commands.has_permissions(administrator=True)
    ...
    async def emojisearch(self, ctx: commands.Context, name: Lower = None):
        ...
        if len(emojis) == 0:
            ...
            return await ctx.reply(
                embed=discord.Embed(
                    description=f"Couldn't find any results for `{name}`, please try again .",
                    color=0x2B2D31,
                )
            )
        ...
        await ctx.reply(
            embed=discord.Embed(
                description=f"Found `{len(emojis)}` emojis .", color=0x2B2D31
            )
        )
    ...
    async def stickersearch(self, ctx: commands.Context, name: Lower = None):
        if not name:
            ctx.command.reset_cooldown(ctx)
            return await ctx.reply(
                embed=discord.Embed(description="Please enter a sticker`")
            )
        stickers = [
        ...
        await ctx.reply(
            embed=discord.Embed(
                description=f"Found `{len(embeds)}` stickers .", color=0x2B2D31
            )
        )
        if len(embeds) == 1:
    ...
    @commands.hybrid_group(
        help="Get info about stickers in a message!",
...
{"**Related Emoji:** "+":"+sticker.emoji+":" if isinstance(sticker, discord.GuildSticker) else "**Tags:** "+', '.join(sticker.tags)}
            ...
            if isinstance(sticker, discord.GuildSticker):
                embed.add_field(
                    name="Guild ID:", value=f"{sticker.guild_id}", inline=False
                )
            else:
...
**Description:** {pack.description}
    ...
    async def kljl(self, ctx, role: discord.Role):
        ...
        await ctx.send("<:tick:1348326381611647046> | Removing roles from all humans")
        for h in humans:
            await h.remove_roles(role)
        await ctx.reply(
            "<:tick:1348326381611647046> | Removed mentioned role from all members"
        )
    ...
    async def kjjk(self, ctx, role: discord.Role):
        ...
        await ctx.send("<:tick:1348326381611647046> | Removing roles from all")
        for h in humans:
            await h.remove_roles(role)
        await ctx.reply("<:tick:1348326381611647046> | Removed mentioned role from all")
    ...
    async def lkklkl(self, ctx, role: discord.Role):
        ...
        await ctx.send(
            "<:tick:1348326381611647046> | Removing roles to all humans & bots"
        )
        for h in humans:
            await h.remove_roles(role)
        await ctx.reply(
            "<:tick:1348326381611647046> | Removed mentioned role from all bots"
        )
...
async def roleall(ctx):
    ...
    embed = Embed(
        ...
        description="Do you want to assign this role to all members in this server?",
        color=0x2B2D31,
    )
    ...
    async def yes_button_click(button_ctx):
        for member in guild.members:
            await member.add_roles(role)
        await button_ctx.send("Role has been assigned to all members.", hidden=True)
    ...
    yes_button = await message.create_button_callback(yes_button_click, custom_id="yes")
    no_button = await message.create_button_callback(no_button_click, custom_id="no")
