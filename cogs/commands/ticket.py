...
async def configdata(guild: discord.Guild):
    ...
    if m_db is None:
        return None
    embed = discord.Embed(title=f"{m_db['name']} Ticket panel setup", color=0x2b2d31).set_footer(text=guild.me.name, icon_url=guild.me.display_avatar.url)
...
class tickredel(discord.ui.View):
    ...
    async def _treopen(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if p is not None:
            ...
            message = f"{user.mention} Welcome Back! The ticket is reopened"
        embed = discord.Embed(description="You will be provided with support shortly\nTo close this ticket click the <:tickett:1209484988513517579> button.", color=0x2b2d31).set_footer(text=f"{guild.me.name} Ticket System", icon_url=guild.me.display_avatar.url)
        v = ticketchannelpanel(self.bot)
        await interaction.channel.send(message, embed=embed, view=v)
        self.bot.add_view(v)
        await interaction.message.edit(view=None)

    @discord.ui.button(label="Delete", emoji="<:ticket_delete:1223594325565509652>", custom_id="delete", style=discord.ButtonStyle.red)
...
class ticketchannelpanel(discord.ui.View):
    ...
    @discord.ui.button(label="Close", emoji="<:mail:1348340617935065240>", custom_id="close", style=discord.ButtonStyle.red)
    async def _ticketchannel(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        em = discord.Embed(title="<:tickett:1209484988513517579> Ticket Closed", description=f"{interaction.channel.mention} is closed by {str(interaction.user)}", color=0x2b2d31).set_footer(text=f"{guild.me.name} Ticket System", icon_url=guild.me.display_avatar.url)
...
class ticketpanel(discord.ui.View):
    ...
    async def _ticket(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if p is not None:
            ...
            message = f"{interaction.user.mention} Welcome"
        embed = discord.Embed(description="You will be provided with support shortly\nTo close this ticket click the <:mail:1348340617935065240> button.", color=0x2b2d31).set_footer(text=f"{guild.me.name} Ticket System", icon_url=guild.me.display_avatar.url)
        v = ticketchannelpanel(self.bot)
...
class editpanelview(discord.ui.View):
    ...
    async def interaction_check(self, interaction: discord.Interaction):
        if interaction.user.id != self.ctx.author.id and interaction.user.id not in  [870179991462236170]:
            ...
            return False
        return True
    @discord.ui.button(label="Support Role", custom_id='support', style=discord.ButtonStyle.grey)
    async def support(self, interaction, button):
        ...
        for i in interaction.guild.roles:
            if not i.is_bot_managed() and not i.is_premium_subscriber():
                xd[i.name] = i.id
        if len(xd) == 0:
            await interaction.message.edit(embed = discord.Embed(title=f"{self.name} Ticket panel setup", description="There are no Roles in the Server", color=0x2b2d31).set_footer(text=f"{self.bot.user.name} Ticket", icon_url=self.bot.user.display_avatar.url))
            await asyncio.sleep(10)
        else:
            x = rolemenuview(self.ctx, "supportrole", "Select the Support Role")
            embed = discord.Embed(title=f"{self.name} Ticket panel setup", description="Select a role which should be allowed to view the ticket channels\nIf a role is not listed below just start typing it's name in select menu box it will be shown as a option", color=0x2b2d31).set_footer(text=f"{self.bot.user.name} Ticket", icon_url=self.bot.user.display_avatar.url)
    ...
    @discord.ui.button(label="Ping Role", custom_id='ping', style=discord.ButtonStyle.grey)
    async def pingrole(self, interaction, button):
        ...
        for i in interaction.guild.roles:
            if not i.is_bot_managed() and not i.is_premium_subscriber():
                xd[i.name] = i.id
        if len(xd) == 0:
            await interaction.edit_original_response(embed = discord.Embed(title=f"{self.name} Ticket panel setup", description="There are no Roles in the Server", color=0x2b2d31).set_footer(text=f"{self.bot.user.name} Ticket", icon_url=self.bot.user.display_avatar.url))
            await asyncio.sleep(10)
        else:
    ...
    @discord.ui.button(label="Open Ticket Category", custom_id='open', style=discord.ButtonStyle.grey)
    async def open(self, interaction, button):
        ...
        for i in interaction.guild.categories:
                xd[i.name] = i.id
        if len(xd) == 0:
            await interaction.edit_original_response(embed = discord.Embed(title=f"{self.name} Ticket panel setup", description="There are no category in the Server", color=0x2b2d31).set_footer(text=f"{self.bot.user.name} Ticket", icon_url=self.bot.user.display_avatar.url))
            await asyncio.sleep(10)
        else:
            x = catmenuview(self.ctx, "opencategory", "Select the Open ticket's category")
            embed = discord.Embed(title=f"{self.name} Ticket panel setup", description="Select a category for open tickets\nIf a category is not listed below just start typing it's name in select menu box it will be shown as a option", color=0x2b2d31).set_footer(text=f"{self.bot.user.name} Ticket", icon_url=self.bot.user.display_avatar.url)
            await interaction.edit_original_response(embed=embed, view=x)
            await x.wait()
        emb = await configdata(interaction.guild)
        await interaction.edit_original_response(embed=emb, view=self)

    @discord.ui.button(label="Closed Ticket Category", custom_id='closed', style=discord.ButtonStyle.grey)
    ...
    @discord.ui.button(label="Embed Message", custom_id='emb', style=discord.ButtonStyle.grey)
    async def embmsg(self, interaction, button):
        embed = discord.Embed(title=f"{self.name} Ticket panel setup", description=f"What should be the Embed message for {self.name}'s Panel", color=0x2b2d31).set_footer(text=f"{self.bot.user.name} Ticket", icon_url=self.bot.user.display_avatar.url)
        def check(message):
    ...
    async def done(self, interaction, button):
        ...
        with sqlite3.connect('data/database.sqlite3') as db:
            ...
            cursor.execute(query, val)
            m_db = cursor.fetchone()
        emb = discord.Embed(title=f"{m_db['name']} Panel", description=m_db['msg'], color=0x2b2d31).set_footer(text=f"{str(self.bot.user.name)} Ticket System", icon_url=self.bot.user.display_avatar.url)
...
class panelview(discord.ui.View):
    ...
    async def interaction_check(self, interaction: discord.Interaction):
        if interaction.user.id != self.ctx.author.id and interaction.user.id not in  [870179991462236170]:
            ...
            return False
        return True
    @discord.ui.button(label="Ticket Channel", custom_id='channel', style=discord.ButtonStyle.grey)
    async def channel(self, interaction: discord.Interaction, button):
        ...
        embed = discord.Embed(title=f"{self.name} Ticket panel setup", description="Where should I setup the ticket panel?\nIf a channel is not listed below just start typing it's name in select menu box it will be shown as a option", color=0x2b2d31).set_footer(text=f"{self.bot.user.name} Ticket", icon_url=self.bot.user.display_avatar.url)
        await interaction.edit_original_response(embed=embed, view=x)
        await x.wait()
    ...
    @discord.ui.button(label="Support Role", custom_id='support', disabled=True, style=discord.ButtonStyle.grey)
    async def support(self, interaction, button):
        await interaction.response.defer(ephemeral=False, thinking=False)
        xd = {}
    ...
    @discord.ui.button(label="Ping Role", custom_id='ping', disabled=True, style=discord.ButtonStyle.grey)
    async def pingrole(self, interaction, button):
        await interaction.response.defer(ephemeral=False, thinking=False)
        xd = {}
        for i in interaction.guild.roles[1:]:
    ...
    @discord.ui.button(label="Open Ticket Category", custom_id='open', disabled=True, style=discord.ButtonStyle.grey)
    async def open(self, interaction, button):
        await interaction.response.defer(ephemeral=False, thinking=False)
        xd = {}
        for i in interaction.guild.categories:
        ...
        if len(xd) == 0:
            ...
            x = catmenuview(self.ctx, "opencategory", "Select the Open ticket's category")
            embed = discord.Embed(title=f"{self.name} Ticket panel setup", description="Select a category for open tickets\nIf a category is not listed below just start typing it in select menu box it's name will be shown as a option", color=0x2b2d31).set_footer(text=f"{self.bot.user.name} Ticket", icon_url=self.bot.user.display_avatar.url)
            await interaction.edit_original_response(embed=embed, view=x)
            await x.wait()
        emb = await configdata(interaction.guild)
        await interaction.edit_original_response(embed=emb, view=self)

    @discord.ui.button(label="Closed Ticket Category", custom_id='closed', disabled=True, style=discord.ButtonStyle.grey)
    async def closed(self, interaction, button):
        await interaction.response.defer(ephemeral=False, thinking=False)
        xd = {}
        for i in interaction.guild.categories:
    ...
    @discord.ui.button(label="Embed Message", custom_id='emb', disabled=True, style=discord.ButtonStyle.grey)
...
class TicketCog(commands.Cog):
    ...
    async def ticket(self, ctx):
        ...
        if prefix == f"<@{self.bot.user.id}> ":
            prefix = "@Ignis "
        ls = ["ticket","ticket setup","ticket delete", "ticket edit", "ticket info", "ticket reopen"]
        des = ""
        for i in sorted(ls):
            cmd = self.bot.get_command(i)
            des += f"`{prefix}{i}`\n{cmd.description}\n\n"
        listem = discord.Embed(title=f"Ticket Commands", colour=0x2b2d31,
    ...
    async def setup(self, ctx: commands.Context):
        print("[DEBUG] ticket setup called")
        if ctx.guild.owner.id == ctx.author.id:
            pass
        else:
            if ctx.author.top_role.position <= ctx.guild.me.top_role.position and ctx.author.id not in  [870179991462236170]:
                em = discord.Embed(description=f"<:cross:1348326385386651748> | You must Have Higher Role than Bot To run This Command", color=0xff0000)
                print("[DEBUG] returning: author top role not higher than bot role")
        ...
        try:
            ...
            with sqlite3.connect('data/database.sqlite3') as db:
                ...
                cursor.execute(query, val)
                m_db = cursor.fetchone()
            print(f"[DEBUG] panel query done, m_db is None: {m_db is None}")
            if m_db is None:
                ...
                print("[DEBUG] returning: panel already exists for this guild")
                return await ctx.send(embed=discord.Embed(description=f"<:cross:1348326385386651748> | You can create only one panel at the moment. ", color=0x2b2d31).set_footer(text=self.bot.user.name, icon_url=self.bot.user.display_avatar.url))
            print("[DEBUG] db insert committed, building view...")
            v = panelview(self.bot, ctx, name)
            print("[DEBUG] calling configdata...")
            emb = await configdata(ctx.guild)
            print(f"[DEBUG] configdata returned embed: {emb is not None}")
            await ctx.reply(embed=emb, view=v)
            print("[DEBUG] reply sent, waiting on view")
            await v.wait()
        except Exception:
            traceback.print_exc()
            raise
    ...
    async def delete(self, ctx: commands.Context):
        ...
        with sqlite3.connect('data/database.sqlite3') as db:
            ...
            m_db = cursor.fetchone()
        if m_db is None:
            return await ctx.send(embed=discord.Embed(description=f"<:cross:1348326385386651748> | No ticket panel found with the name `{name}`", color=0x2b2d31).set_footer(text=self.bot.user.name, icon_url=self.bot.user.display_avatar.url))
        try:
            c = self.bot.get_channel(m_db["channel_id"])
            m = await c.fetch_message(m_db['msg_id'])
        ...
        return await ctx.send(embed=discord.Embed(description=f"<:tick:1348326381611647046> | Successfully deleted ticket panel with name `{name}`", color=0x2b2d31).set_footer(text=self.bot.user.name, icon_url=self.bot.user.display_avatar.url))
