...
class AutoBlacklist(Cog):
    def __init__(self, client: Ignis):
      ...
      self.spam_command_mapping = commands.CooldownMapping.from_cooldown(6, 8, commands.BucketType.member)
    ...
    async def on_message(self, message):
      ...
      if retry:
        if message.content == Ignis or message.content == "<@1355539303475712032>":
          add_user_to_blacklist(message.author.id)
          embed = discord.Embed(description="**Successfully Blacklisted {} For Spam Mentioning Me!**".format(message.author.mention),color=0x2b2d31)
    ...
    async def on_command(self, ctx):
      ...
      if retry:
        ...
        embed = discord.Embed(description="**Successfully Blacklisted {} For Spamming My Commands!**".format(ctx.author.mention),color=0x2b2d31)
        embed.set_thumbnail( url=self.client.user.display_avatar.url)
