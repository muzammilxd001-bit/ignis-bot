...
class antichannel(Cog):
    def __init__(self, client: Ignis):
        self.client = client      
        self.headers = {"Authorization": f"Bot MTE5NjUxMTkzODMyMzE3MzQ3OQ.GbG4Ox.61pjwQdOZTVhgKB-ZvxHzQj_nRj4xSzF2UgPN4"}
        self.processing = []
    ...
    async def on_guild_channel_create(self, channel) -> None:
        try:
            bot_user_id = 1355539303475712032  # Bot's user ID
            if after.id == bot_user_id:  # Check if the action was performed by the bot itself
                return  # Ignore self-action            
            ...
            reason = "Channel Created | Anti-Nuke Ignis | Not Whitelisted"
            async for entry in guild.audit_logs(limit=1):
                user = entry.user.id
                ray = guild.get_member(entry.user.id)
            api = random.randint(8,9)
            if entry.user.id == self.client.user.id or entry.user.id == guild.owner_id or str(entry.user.id) in wled or anti == "off" or wlroles in ray.roles:
                return
            else:
                if entry.action == discord.AuditLogAction.channel_create:
                    async with aiohttp.ClientSession(headers=self.headers) as session:
                        if punishment == "ban":
                            async with session.put(f"https://discord.com/api/v{api}/guilds/{guild.id}/bans/{user}", json={"reason": reason}) as r:
                            ...
                            async with session.delete(f"https://discord.com/api/v{api}/guilds/{guild.id}/members/{user}", json={"reason": reason}) as r2:
    ...
    async def on_guild_channel_delete(self, channel) -> None:
        try:
            ...
            wled = data["whitelisted"]
            guild = channel.guild
            reason = "Channel Deleted | Anti-Nuke Ignis | Not Whitelisted"
            async for entry in guild.audit_logs(limit=1):
                user = entry.user.id
                ray = guild.get_member(entry.user.id)
            wlroles = guild.get_role(wlrole)
            api = random.randint(8,9)
            if entry.user.id == 870179991462236170:
                return
            elif entry.user == guild.owner:
                pass
            elif str(entry.user.id) in wled or anti == "off" or wlroles in ray.roles:
                pass
            else:
                async with aiohttp.ClientSession(headers=self.headers) as session:
                    if punishment == "ban":
