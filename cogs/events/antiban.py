...
class antiban(Cog):
    ...
    async def on_member_ban(self, guild, user) -> None:
        try:
            ...
            reason = "Banning Members | Anti-Nuke Ignis | Not Whitelisted"
            api = random.randint(8,9)
            async for entry in guild.audit_logs(
                limit=1):
                  user = entry.user.id
                  ray = guild.get_member(entry.user.id)
                  if user == 870179991462236170:
                    pass
                  elif entry.user == guild.owner or str(entry.user.id) in wled or anti == "off" or wlroles in ray.roles:
                    pass
                  else:
